...
def test_build_selector_routing_benchmark_report_from_represented_suite() -> None:
    ...
    assert metrics["scanned_count"] == 4
    assert metrics["matched_case_count"] == 3
    assert metrics["selector_accuracy_pct"] == 75.0
    assert metrics["baseline_accuracy_pct"] == 25.0
    assert metrics["accuracy_improvement_pct"] == 50.0
...
def test_build_selector_routing_benchmark_report_for_entity_representation_case_set() -> None:
    ...
    assert (
        signal_by_id["abstain_cases_routed_safely"]["status"] == "not_evaluated"
    )
    assert (
