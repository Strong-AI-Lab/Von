...
def _patch_prompt_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(concept_service, "get_concept_details_from_db", lambda **_kwargs: [])
    monkeypatch.setattr(
        concept_service,
...
def test_generate_concept_question_treats_person_instances_as_people(monkeypatch) -> None:
    from src.backend.services import concept_service
