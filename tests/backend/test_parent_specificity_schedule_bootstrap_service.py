"""Tests for parent-specificity schedule bootstrap service (JVNAUTOSCI-217)."""
...
def test_bootstrap_creates_schedule_when_missing(monkeypatch) -> None:
    ...
    assert created_schedule.workflow_id == mod.PARENT_SPECIFICITY_RUMINATION_WORKFLOW_ID
    assert created_schedule.default_inputs.get("managed_schedule_key") == (
        mod.PARENT_SPECIFICITY_SCHEDULE_MANAGED_KEY
    )
