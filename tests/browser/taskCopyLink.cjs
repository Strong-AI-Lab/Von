...
const output = path.resolve(process.argv[2] || '.run/task-copy-link');
const task = { task_concept_id: '#V#task_agent_8f35430789702d95864e5def732c43cd', title: 'Represent deployed Von builds and link deployments to tasks', description: 'long_unbroken_research_reference_'.repeat(12), status: 'completed', priority: 'medium' };
...
(async () => {
    ...
    try {
        ...
        for (const viewport of [{ width: 1440, height: 900 }, { width: 390, height: 844 }, { width: 320, height: 568 }]) {
            const context = await browser.newContext({ viewport, hasTouch: viewport.width < 500, permissions: ['clipboard-read', 'clipboard-write'] });
            const page = await context.newPage();
            page.on('pageerror', error => { errors.push(error.message); console.error(error.message); });
            await page.goto(`http://127.0.0.1:${server.address().port}/`);
            await page.getByRole('button', { name: 'Task reference', exact: true }).click();
            const panel = page.getByRole('dialog', { name: 'Conversation tasks', exact: true });
            await expect(panel.locator('.task-detail-toggle-btn')).toHaveAttribute('aria-expanded', 'true');
            const copy = panel.getByRole('button', { name: 'Copy link', exact: true });
            const hitTarget = await copy.boundingBox();
            const minimumTarget = viewport.width < 500 ? 44 : 32;
            assert(hitTarget.width >= minimumTarget && hitTarget.height >= minimumTarget);
            await copy.focus();
            await copy.press('Enter');
            await expect(panel.getByRole('status')).toHaveText('Link copied to clipboard');
            const copiedUrl = await page.evaluate(() => navigator.clipboard.readText());
            assert.equal(new URL(copiedUrl).searchParams.get('task'), task.task_concept_id);
            assert.equal(new URL(copiedUrl).hash, '#globalTasksTab');
            ...
            await pageCopy.click();
            await expect(inspector.getByRole('status')).toHaveText('Copy failed');
            await expect(page.locator('.toast-error')).toHaveText('Copy failed');
            await page.evaluate(() => { navigator.clipboard.writeText = window.originalWrite; });
            await pageCopy.click();
            await expect(inspector.getByRole('status')).toHaveText('Link copied to clipboard');
            await page.screenshot({ path: path.join(output, `page-${viewport.width}.png`) });
            await page.goto(copiedUrl);
            await expect(page.locator('#globalTaskInspector .task-inspector-card')).toBeVisible();
            await expect(page.locator('#globalTaskInspector .task-inspector-card')).toHaveAttribute('data-task-id', task.task_concept_id);
            await expect(page.locator('.task-item.is-selected')).toHaveAttribute('data-task-id', task.task_concept_id);
            measurements.push({ viewport, hitTarget, before, after, overflow, bodyOverflow, copiedUrl });
            await context.close();
        }
