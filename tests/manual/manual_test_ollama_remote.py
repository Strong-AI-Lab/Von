try:  # Optional dependency; script provides guidance if missing
    ...
    print("[WARN] 'ollama' package not installed. Install with 'pip install ollama' to use this script.", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if models['models']:
            ...
            for model_item in models['models']: # Iterate through the list of model details
                # Try to get 'name', then 'model'. If both fail, use a placeholder.
                model_identifier = model_item.get('name')
                if not model_identifier:
                    model_identifier = model_item.get('model', f"Identifier missing for item: {model_item}")
                print(f"- {model_identifier}")

            # Example: Run a simple completion
            if models['models']: # Ensure list is not empty before trying to access its first element
                ...
                if model_to_use:
                    ...
                    response = client.generate(model=model_to_use, prompt=prompt, stream=False) # Set stream=False for simplicity

                    print("\nResponse:")
                    print(response['response'])
                else:
                    print(f"Could not determine model to use for completion. Details of the first model: {first_model_item}", file=sys.stderr)

        else:
            print("Successfully connected to Ollama server, but no models are available.")
            print("Please pull a model on the server machine using 'ollama pull <model_name>'")
        ...
        if ollama is not None and getattr(ollama, 'ResponseError', None) and isinstance(e, ollama.ResponseError):  # type: ignore[attr-defined]
            ...
            print("Ensure the model exists on the server and the server is running correctly.", file=sys.stderr)
        else:
            print(f"Failed to connect to Ollama server at {OLLAMA_SERVER_URL}", file=sys.stderr)
            print(f"Error: {e}", file=sys.stderr)
            print("\nPlease check the following:", file=sys.stderr)
            print(f"1. The server script 'start_ollama_remote.py' is running on the server machine.", file=sys.stderr)
            print(f"2. The server machine's firewall allows incoming TCP connections on port 11434.", file=sys.stderr)
            print(f"3. The IP address '{OLLAMA_SERVER_URL}' is correct and reachable from this client machine.", file=sys.stderr)
            if "Connection refused" in str(e):
                print("   (Error: Connection refused suggests a firewall issue or Ollama not running/listening on that address)", file=sys.stderr)
            elif "timed out" in str(e):
                print("   (Error: Connection timed out suggests a network routing or firewall issue)", file=sys.stderr)
