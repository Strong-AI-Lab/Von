...
- For local Von backend restarts, use the repo launcher path (`.\run.ps1 restart -NoBrowser -HealthTimeoutSec 180`) rather than manually spawning `src/workflows/von/main.py`. For automated replay or coding-agent acceptance that should not disturb a user-facing local server, use the isolated launcher path (`.\run.ps1 restart -AgentTest -HealthTimeoutSec 180`) and point replay tools at `http://127.0.0.1:5010` unless a different `-Port` is explicitly supplied.
