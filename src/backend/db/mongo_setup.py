...
def get_application_settings_collection() -> Collection | None:
    ...
    if db is None:
        print("Error: Could not get database instance (via mongo_client.get_db()). Cannot access application_settings collection.")
        return None
    ...
    try:
        ...
        print(f"MongoDB operation failed while accessing or indexing '{APPLICATION_SETTINGS_COLLECTION_NAME}': {e}")
        return None
    except Exception as e: # Catch any other unexpected errors
        print(f"An unexpected error occurred while getting application settings collection: {e}")
        return None
