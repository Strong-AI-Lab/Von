...
def health_summary() -> Dict[str, Any]:
    ...
    for k, v in _state.items():
        if k.startswith('reconnect_') or k.startswith('last_') or k == 'consecutive_failures':
    ...
    try:
        ...
        using_fallback = bool(using_fallback_raw) if isinstance(using_fallback_raw, (bool, int)) or using_fallback_raw is not None else None
    except Exception:
        using_fallback = None
    try:
...
def attempt_reconnect(force: bool = False, max_attempts: int = 5, base_ms: int = 1000, max_ms: int = 30000) -> Dict[str, Any]:
    with _lock:
    ...
    while attempt_num < max_attempts:
        ...
        try:
            ...
            with _lock:
                ...
                _state['last_connected_at'] = _now()
            return {'skipped': False, 'reconnected': True, 'attempts': attempt_num, **health_summary()}
...
def _monitor_loop(interval_sec: int, base_ms: int, max_ms: int, max_attempts: int):
    while True:
        ...
        h = health_summary()
        if not h['connected']:
            attempt_reconnect(force=False, max_attempts=max_attempts, base_ms=base_ms, max_ms=max_ms)


def ensure_monitor_started(interval_sec: int = 15, base_ms: int = 1000, max_ms: int = 30000, max_attempts: int = 5):
    global _monitor_started, _monitor_thread
    with _lock:
        if _monitor_started:
            return
        _monitor_started = True
    t = threading.Thread(target=_monitor_loop, args=(interval_sec, base_ms, max_ms, max_attempts), daemon=True, name='MongoReconnectMonitor')
    t.start()
    _monitor_thread = t
