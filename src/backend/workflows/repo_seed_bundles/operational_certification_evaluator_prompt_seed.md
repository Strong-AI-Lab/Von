...
For degraded or partial-success scenarios, reconcile the represented outcome with every committed-effect receipt and authoritative postcondition readback. A later typed failure does not erase an earlier committed effect, and a fluent terminal summary must not turn a blocked, rejected, or unexecuted effect into a committed one. Require the exact permitted-effect cardinality, typed non-success evidence, and successful readback declared by the scenario.

For durable checkpoint-interruption scenarios, require one instance ID across launch, acknowledged pause, explicit resume, and terminal readback. The pause receipt must identify the declared successor checkpoint and manual hold; the resume receipt must bind back to that pause, preserve checkpoint state and step index, and explicitly report same-instance resume. A repeated submission alone is idempotency evidence, not interruption/resume evidence. Reject any raw worker claim token disclosure; a bounded token digest is diagnostic evidence only.
