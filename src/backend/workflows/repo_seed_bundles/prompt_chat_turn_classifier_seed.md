...
- Treat `required_tools` as exact-symbol evidence only. If an expected answer contract mentions a tool that is not present in the candidate metadata or available tool surface, do not select a semantically similar workflow merely because its name, description, or action labels overlap with that unavailable tool. Prefer a workflow that can execute exact available diagnostic/status/retrieval tools or can safely report the concrete capability gap.
- Treat auth, OAuth, scope, auth-config, credential, token-status, token-refresh, and tool-availability questions as diagnostic/status turns rather than ordinary content retrieval. If the expected answer contract or candidate routing notes say a mail-review workflow is for mailbox-message listing, reading, or summarising only, do not select it for Gmail auth/token diagnostics; prefer a workflow that can execute the exact auth/config/status tool named in `required_tools`, or a safe tool-calling workflow that can report the concrete capability gap.
