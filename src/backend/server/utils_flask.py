...
def create_flask_app(
    # --- Updated Type Hints ---
    list_models_func: Callable[[], List[str]], # Expects a function returning list of strings
    generate_func: Callable[[str, Optional[List[Dict[str, str]]], Optional[str]], str], # Matches LLMInterface.generate signature (prompt, context, model)
    static_folder_path: str = os.path.join(project_root, "src", "frontend", "web", "von_interface", "static"),
    template_folder_path: str = os.path.join(project_root, "src", "frontend", "web", "von_interface", "templates")
) -> Flask:
    ...
    app = Flask(__name__,
                ...
                template_folder=template_folder_path) # Template folder set here is default, Blueprint can override

    # --- Configuration Setup ---
    # Set secret key for session management (required for Google OAuth)
    app.secret_key = os.environ.get('FLASK_SECRET_KEY', 'von-dev-secret-key-change-in-production')
    ...
    app.config.setdefault("MODEL", "granite3.3:2b") # Keep default for now, but be aware

    # --- Register Blueprints ---
    app.register_blueprint(von_bp, url_prefix='/von') # MODIFIED
    app.register_blueprint(vontology_bp, url_prefix='/vontology/api/vontology') # MODIFIED: Full prefix
    app.register_blueprint(concept_bp, url_prefix='/api/concepts') # Register the new concept blueprint with prefix
    app.register_blueprint(settings_bp, url_prefix='/api/settings') # Register the new settings blueprint with prefix
    app.register_blueprint(elicitation_bp, url_prefix='/api/elicitation') # Register the new elicitation blueprint with prefix
    ...
    app.register_blueprint(auth_bp, url_prefix='/von') # Register the new auth blueprint with /von prefix to match Google OAuth config
    # ---------------------------
    ...
    try:
        ...
        print(f"--- Flask App Initializing - Version: {APP_VERSION} ---") # Fallback print
    ...
    try:
        ...
        internal_mcp_enabled = os.getenv('VON_INTERNAL_MCP_ENABLE', '0').lower() in {'1', 'true'}
        ...
        if internal_mcp_enabled:
            app.logger.info("[mcp_gateway] Enabled with %d registered methods.", method_count)
        else:
            app.logger.info("[mcp_gateway] Initialised (disabled). Set VON_INTERNAL_MCP_ENABLE=1 to activate. Methods=%d", method_count)
    ...
    try:
        ...
        if not pc_status.get('available'):
            app.logger.warning(
                ...
                PROMPT_CONCEPT_ID, pc_status.get('source_field'), pc_status.get('error')
            )
        else:
            app.logger.info(
                "Prompt concept %s OK (source_field=%s)", PROMPT_CONCEPT_ID, pc_status.get('source_field')
            )
    ...
    app.config['SERVER_START_TIME'] = app.config.get('SERVER_START_TIME') or datetime.now(_tz.utc).isoformat().replace('+00:00', 'Z')
    ...
    def health_check():
        ...
        if not public_ip:
            try:
                ...
                with urllib.request.urlopen('https://api.ipify.org?format=text', timeout=3) as response:
    ...
    def diagnostics():
        ...
        try:
            ...
            try:
                ...
                if tracemalloc.is_tracing():
                    ...
                    rss_mb = round(sum([s.size for s in snap.statistics('filename')]) / (1024*1024), 2)
            except Exception:
                pass
        ...
        try:
            ...
            uptime_sec = (datetime.now(_tz.utc) - start_dt).total_seconds() if start_dt else None
        except Exception:
            uptime_sec = None
        ...
        try:
            ...
            now_ts = time.time(); ttl_env = os.getenv('VONTOLOGY_TREE_TTL')
            if _TREE_CACHE:
        ...
        try:
            ...
            if isinstance(cache_obj, dict):
                split_entries = sum(1 for key in cache_obj.keys() if isinstance(key, str) and key.endswith('|split'))
                for value in cache_obj.values():
                    ...
                    if not isinstance(raw_map, dict):
                        raw_map = {}
                    sample_scope = {
                        'predicates_by_scope_keys': list((scope_payload.get('predicates_by_scope') or {}).keys()),
                        'predicate_origin_keys': list((scope_payload.get('predicate_origins') or {}).keys()),
                        'raw_scope_counts': {k: len(v) if isinstance(v, (list, set, tuple)) else 0 for k, v in raw_map.items()}
                    }
                    break
            salient_cache = {
                'size': cache_size,
        ...
        try:
            ...
            try:
                ...
                if coll is not None:
                    total_user_specific = coll.count_documents({'relationships.specific_to_user': {'$exists': True, '$ne': []}})
                    if effective_user:
                        visible_user_specific = coll.count_documents({'relationships.specific_to_user': effective_user})
                    else:
    ...
    def admin_shutdown():
        ...
        if func is None:
            ...
            try:
                ...
                return jsonify(success=False, error='shutdown_failed', detail=str(e)), 500
        else:
            func()
    ...
    def _start_prewarm():  # local closure
        try:
            if os.getenv('VON_PREWARM_DISABLE') in ('1','true','TRUE','True') or app.config.get('PREWARM_DISABLE'):
            ...
            def _prewarm_worker():
                ...
                try:
                    try:
                        ...
                        if callable(lm_func):
                            models = lm_func(); app.logger.info('[prewarm] Listed %d models.', len(models) if isinstance(models, list) else -1)
                    except Exception as e:
                    ...
                    try:
                        with app.test_client() as c:
                            ...
                            if r.status_code == 200:
                                app.logger.info('[prewarm] Tree build OK (len bytes=%s)', len(r.data))
                            else:
                                app.logger.warning('[prewarm] Tree build non-200 status=%s', r.status_code)
                    except Exception as e:
                        app.logger.warning('[prewarm] Tree build failed: %s', e)
                finally:
                    app.logger.info('[prewarm] Completed in %.2fs', _time.time() - t0)
            threading.Thread(target=_prewarm_worker, name='prewarm-thread', daemon=True).start()
        except Exception as e:
            try: app.logger.warning('[prewarm] Failed to start: %s', e)
            except Exception: pass

    if not app.testing and 'PYTEST_CURRENT_TEST' not in os.environ and not app.config.get('PREWARM_DISABLE'):
        try:
...
if __name__ == '__main__':
    ...
    def dummy_generate_func(prompt: str, context: Optional[List[Dict[str, str]]], model: Optional[str]) -> str:
