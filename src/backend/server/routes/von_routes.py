...
def _submit_generate_conversation_turn_instance(
    *,
    ...
    submission = None
    try:
        state.manager = get_instance_manager()
        submission = submit_verified_workflow_instance(
            ...
            event_idempotency_key=f"von.generate:conversation_turn:{session_id}:{request_id}",
        )
        submission_payload = submission.to_dict()
    ...
    *,
    ...
    if enhanced_context is not None:
        try:
            ...
            context_stats = {
                "sent_to_llm": _calculate_context_stats(normalised_context)
            }
        except Exception:
            context_stats = None
    ...
    normalised_response_transformations = (
        dict(response_transformations)
        if isinstance(response_transformations, Mapping)
        else build_response_transformation_telemetry_payload(request_id=request_id)
    )
    ...
    return _finalise_llm_debug_info(
