    ...
    try:
        ...
        if source_filter in ('text_relations', 'all'):
            text_rel_items, text_rel_count = _query_text_relations_extent(
                concept_id, subject_type, object_type, limit, offset, sort_by, sort_order
            )
        ...
        current_app.logger.error(
            f'Failed to get extent for predicate {concept_id}: {e}',
            exc_info=True
        )
...
def get_predicate_metadata(concept_id: str):
    ...
    try:
        ...
        current_app.logger.error(
            f'Failed to get metadata for predicate {concept_id}: {e}',
            exc_info=True
        )
...
def _query_text_relations_extent(
    ...
    try:
        ...
        cursor = text_rel_coll.find(query_filter).sort(sort_criteria).skip(offset).limit(limit)

        extent_items = []
        for rel in cursor:
            # Fetch the text value
            text_value = TextValuesRepository.find_one({'_id': ObjectId(rel.get('object_text_id'))})
        ...
        current_app.logger.error(f'Error querying text_relations extent: {e}', exc_info=True)
        return [], 0
...
def _query_structured_relations_extent(
    ...
    try:
        ...
        query_filter: Dict[str, Any] = {f'relationships.{predicate_concept_id}': {'$exists': True, '$ne': None}}
        ...
        for concept in cursor:
            ...
            for obj_value in object_values:
                obj_name = _get_concept_name(obj_value) if isinstance(obj_value, str) else None
        ...
        current_app.logger.error(f'Error querying structured relations extent: {e}', exc_info=True)
        return [], 0


def _sort_extent_items(items: List[Dict[str, Any]], sort_by: str, sort_order: str) -> List[Dict[str, Any]]:
...
def _find_meta_fact(meta_predicate: str, subject: str, concepts_coll) -> Optional[Dict[str, Any]]:
...
def _find_text_relations_for_predicate(subject_id: str, predicate: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        relations = list(text_rel_coll.find({
...
def _find_custom_meta_properties(predicate_id: str, concepts_coll) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        excluded_preds = {'is_a_type_of', 'is_an_instance_of', 'most_salient_type', '#V#salient_binary_predicate_for_type'}
...
def _calculate_extent_statistics(predicate_id: str) -> Dict[str, int]:
    ...
    try:
        ...
        if text_rel_coll is not None:
            ...
            # Get unique subjects
            unique_subjects = text_rel_coll.distinct('subject_concept_id', {'predicate': predicate_id})
        ...
        current_app.logger.error(f'Error calculating extent statistics: {e}', exc_info=True)
