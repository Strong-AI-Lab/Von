...
from ...services.concept_service import ConceptServiceError, InvalidConceptDataError, ConceptNotFoundError, import_concepts as import_concepts_service
...
from ...vontology.utils_vontology import get_vontology_node_and_descendant_ids, get_concept_notes # Added getter import
from ...db.mongo_client import get_db
from bson import ObjectId # Required for converting string IDs if necessary, though service should handle
...
def list_concepts_route():
    ...
    try:
        ...
        current_app.logger.error(f"Unexpected error listing concepts: {e}", exc_info=True)
...
def create_concept_route():
    ...
    try:
        ...
        current_app.logger.error(f"Unexpected error creating concept: {e}", exc_info=True)
        return jsonify(error="An unexpected error occurred"), 500
    ...
    if request.method == 'GET':
        try:
            ...
            if concept:
                ...
                concept = enrich_concept_with_text_relations(concept, logger=current_app.logger)
                ...
                if descriptions_from_relations:
                    ...
                    concept['description'] = descriptions_from_relations[0].get('text', '')
                else:
        ...
        except InvalidConceptDataError as e: # Should not happen for GET by ID if ID format is validated by service
            current_app.logger.warning(f"Invalid data for getting concept (ID: {concept_id}): {e}")
            return jsonify(error=str(e)), 400
        except ConceptServiceError as e:
            current_app.logger.error(f"Service error getting concept {concept_id}: {e}", exc_info=True)
            return jsonify(error=str(e)), 500
        except Exception as e:
            current_app.logger.error(f"Unexpected error getting concept {concept_id}: {e}", exc_info=True)
            return jsonify(error="An unexpected error occurred"), 500
        ...
        try:
            ...
            if 'notes' in data and len(data) == 1:
                ...
                success = concept_service.update_concept_description(concept_id, data['description'])
                if success:
                    if concept_id.startswith('#V#'):
                        updated_concept = concept_service.get_concept_by_concept_id(concept_id)
                    else:
            ...
            current_app.logger.info(f"concept not found for update (ID: {concept_id}): {e}")
            return jsonify(error=str(e)), 404
        except InvalidConceptDataError as e:
            current_app.logger.warning(f"Invalid data for updating concept (ID: {concept_id}): {e}")
            return jsonify(error=str(e)), 400
        except ConceptServiceError as e:
            current_app.logger.error(f"Service error updating concept {concept_id}: {e}", exc_info=True)
            return jsonify(error=str(e)), 500
        except Exception as e:
            current_app.logger.error(f"Unexpected error updating concept {concept_id}: {e}", exc_info=True)
            return jsonify(error="An unexpected error occurred"), 500
        ...
        try:
            ...
            if success:
                return jsonify(message="concept deleted successfully"), 200 # Or 204 No Content
            else:
                # This case should ideally be covered by ConceptNotFoundError if service handles it
                current_app.logger.warning(f"concept not found for deletion or delete failed (ID: {concept_id})")
                return jsonify(error="concept not found or delete operation failed"), 404
        except ConceptNotFoundError as e:
            current_app.logger.info(f"concept not found for deletion (ID: {concept_id}): {e}")
            return jsonify(error=str(e)), 404
        except InvalidConceptDataError as e: # Should not happen for DELETE by ID if ID format is validated
            current_app.logger.warning(f"Invalid data for deleting concept (ID: {concept_id}): {e}")
            return jsonify(error=str(e)), 400
        except ConceptServiceError as e:
            current_app.logger.error(f"Service error deleting concept {concept_id}: {e}", exc_info=True)
            return jsonify(error=str(e)), 500
        except Exception as e:
            current_app.logger.error(f"Unexpected error deleting concept {concept_id}: {e}", exc_info=True)
            return jsonify(error="An unexpected error occurred"), 500
...
def export_concepts_route():
    ...
    try:
        concept_id = request.args.get('concept_id')
        include_descendants_str = request.args.get('include_descendants', 'true').lower()
        ...
        if include_descendants_str in ['true', '1', 'yes']:
            ...
            return jsonify(error="include_descendants parameter must be 'true' or 'false'"), 400

        # Log the export request
        current_app.logger.info(f"concept export requested: concept_id={concept_id}, include_descendants={include_descendants}, format={format_param}")
        ...
        return jsonify({
            ...
            "export_metadata": {
                ...
                "include_descendants": include_descendants,
                "format": format_param,
                "exported_at": datetime.now(dt.timezone.utc).isoformat().replace('+00:00', 'Z')
            }
        }), 200
        ...
        current_app.logger.error(f"Service error exporting concepts: {e}", exc_info=True)
        return jsonify(error=str(e)), 500
    except ValueError as e:
        current_app.logger.warning(f"Invalid parameter value in export: {e}")
        return jsonify(error=f"Invalid parameter value: {e}"), 400
    except Exception as e:
        current_app.logger.error(f"Unexpected error exporting concepts: {e}", exc_info=True)
...
def import_concepts_route():
    ...
    try:
        ...
        conflict_resolution = (request.form.get('conflict_resolution') or 'update').strip().lower()
        validate_flag = (request.form.get('validate_concepts') or 'true').strip().lower() in ['1','true','yes','on']
        dry_run = (request.form.get('dry_run') or 'false').strip().lower() in ['1','true','yes','on']
        ...
        current_app.logger.error(f"Service error importing concepts: {e}", exc_info=True)
        return jsonify(error=str(e)), 500
    except Exception as e:
        current_app.logger.error(f"Unexpected error importing concepts: {e}", exc_info=True)
...
def get_legacy_import_metrics():
    ...
    try:
        ...
        include_reads_requested = 'reads' in [p.strip().lower() for p in include_param.split(',') if p]
        ...
        if not read_doc.get('legacy_read_fields'):
            try:
                ...
                if _LEGACY_READ_FALLBACK_DOC:
                    ...
                    read_doc = {
                        'legacy_read_fields': {k: v for k, v in _LEGACY_READ_FALLBACK_DOC.items() if v}
                    }
            except Exception:
                pass
        ...
        return jsonify({"success": False, "error": "Failed to fetch legacy metrics"}), 500
...
def start_concept_interaction_route(concept_id: str):
    ...
    try:
        ...
        if not user_id:
            return jsonify(error="Missing user context: provide X-User-Client-ID header or establish session"), 400

        initial_notes = request.json.get('initial_notes', '') if request.json else ''

        result = concept_service.start_interaction_session(concept_id, user_id=user_id, initial_notes=initial_notes)
        return jsonify(result), 200
    except ConceptNotFoundError as e:
        current_app.logger.info(f"concept not found for starting interaction (ID: {concept_id}): {e}")
        return jsonify(error=str(e)), 404
    except InvalidConceptDataError as e:
        current_app.logger.warning(f"Invalid data for starting interaction with concept (ID: {concept_id}): {e}")
        return jsonify(error=str(e)), 400
    except ConceptServiceError as e:
        current_app.logger.error(f"Service error starting interaction with concept {concept_id}: {e}", exc_info=True)
        return jsonify(error=str(e)), 500
    except Exception as e:
        current_app.logger.error(f"Unexpected error starting interaction with concept {concept_id}: {e}", exc_info=True)
        return jsonify(error="An unexpected error occurred"), 500
...
def generate_initial_question_route(concept_id: str):
    ...
    try:
        ...
        initial_notes = request.json.get('initial_notes', '') if request.json else ''
        question = concept_service.generate_initial_question(concept_id, initial_notes=initial_notes)
        return jsonify({"question": question}), 200
    except ConceptNotFoundError as e:
        current_app.logger.info(f"concept not found for generating initial question (ID: {concept_id}): {e}")
        return jsonify(error=str(e)), 404
    except ConceptServiceError as e:
        current_app.logger.error(f"Service error generating initial question for concept {concept_id}: {e}", exc_info=True)
        return jsonify(error=str(e)), 500
    except Exception as e:
        current_app.logger.error(f"Unexpected error generating initial question for concept {concept_id}: {e}", exc_info=True)
        return jsonify(error="An unexpected error occurred"), 500
...
def update_concept_notes_route(concept_id: str):
    ...
    if 'notes' not in data or not isinstance(data['notes'], str):
        return jsonify(error="Request body must be JSON with a 'notes' field (string)"), 400
    try:
        ...
        current_app.logger.info(f"concept not found for notes update (ID: {concept_id}): {e}")
        return jsonify(error=str(e)), 404
    except InvalidConceptDataError as e:
        current_app.logger.warning(f"Invalid data for notes update (ID: {concept_id}): {e}")
        return jsonify(error=str(e)), 400
    except ConceptServiceError as e:
        current_app.logger.error(f"Service error updating notes for concept {concept_id}: {e}", exc_info=True)
        return jsonify(error=str(e)), 500
    except Exception as e:
        current_app.logger.error(f"Unexpected error updating notes for concept {concept_id}: {e}", exc_info=True)
        return jsonify(error="An unexpected error occurred"), 500
...
def update_concept_description_route(concept_id: str):
    ...
    if 'description' not in data or not isinstance(data['description'], str):
        return jsonify(error="Request body must be JSON with a 'description' field (string)"), 400
    try:
        success = concept_service.update_concept_description(concept_id, data['description'])
        if success:
            if concept_id.startswith('#V#'):
        ...
        current_app.logger.info(f"concept not found for description update (ID: {concept_id}): {e}")
        return jsonify(error=str(e)), 404
    except InvalidConceptDataError as e:
        current_app.logger.warning(f"Invalid data for description update (ID: {concept_id}): {e}")
        return jsonify(error=str(e)), 400
    except ConceptServiceError as e:
        current_app.logger.error(f"Service error updating description for concept {concept_id}: {e}", exc_info=True)
        return jsonify(error=str(e)), 500
    except Exception as e:
        current_app.logger.error(f"Unexpected error updating description for concept {concept_id}: {e}", exc_info=True)
        return jsonify(error="An unexpected error occurred"), 500
...
def submit_concept_answer_route(concept_id: str):
    ...
    try:
        ...
        if not session:
            current_app.logger.warning(f"Interaction session not found for ID: {interaction_id}")
            return jsonify(error="Interaction session not found"), 404

        # Call the service to submit the answer
        result = concept_service.submit_concept_answer(interaction_id=interaction_id, user_answer=answer, session=session)
        return jsonify(result), 200

    except ConceptNotFoundError as e:
        current_app.logger.info(f"concept or Interaction not found for submitting answer (concept ID: {concept_id}, Interaction ID: {interaction_id}): {e}")
        return jsonify(error=str(e)), 404
    except InvalidConceptDataError as e:
        current_app.logger.warning(f"Invalid data for submitting answer (concept ID: {concept_id}, Interaction ID: {interaction_id}): {e}")
        return jsonify(error=str(e)), 400
    except ConceptServiceError as e:
        current_app.logger.error(f"Service error submitting answer for concept {concept_id}, interaction {interaction_id}: {e}", exc_info=True)
        return jsonify(error=str(e)), 500
    except Exception as e:
        current_app.logger.error(f"Unexpected error submitting answer for concept {concept_id}, interaction {interaction_id}: {e}", exc_info=True)
        return jsonify(error="An unexpected error occurred"), 500
...
def get_active_interactions_route(concept_id: str):
    ...
    try:
        ...
        if not user_id:
            return jsonify(error="Missing user context: provide X-User-Client-ID header or establish session"), 400
        active_interactions = concept_service.get_active_interactions_for_concept(concept_id, user_id)
        ...
        current_app.logger.info(f"concept not found for active interactions check (ID: {concept_id}): {e}")
        return jsonify(error=str(e)), 404
    except ConceptServiceError as e:
        current_app.logger.error(f"Service error getting active interactions for concept {concept_id}: {e}", exc_info=True)
        return jsonify(error=str(e)), 500
    except Exception as e:
        current_app.logger.error(f"Unexpected error getting active interactions for concept {concept_id}: {e}", exc_info=True)
        return jsonify(error="An unexpected error occurred"), 500
...
def resume_interaction_route(concept_id: str):
    ...
    try:
        ...
        current_app.logger.info(f"concept or interaction not found for resume (concept ID: {concept_id}): {e}")
        return jsonify(error=str(e)), 404
    except ConceptServiceError as e:
        current_app.logger.error(f"Service error resuming interaction for concept {concept_id}: {e}", exc_info=True)
        return jsonify(error=str(e)), 500
    except Exception as e:
        current_app.logger.error(f"Unexpected error resuming interaction for concept {concept_id}: {e}", exc_info=True)
        return jsonify(error="An unexpected error occurred"), 500
...
def get_concept_texts_route(concept_id: str):
    ...
    try:
        ...
        current_app.logger.error(f"Error fetching texts for concept {concept_id}: {e}", exc_info=True)
...
def upsert_concept_text_route(concept_id: str):
    ...
    if predicate == RelationPredicate.HAS_NAME:
        ...
        if name_type is None:
            ...
            if name_type_normalised not in {'NL', 'ABBR', 'CODE'}:
                return jsonify(error="context.name_type must be one of NL, ABBR, CODE"), 400
    ...
    try:
        ...
        if result.get('context_updated'):
            status = 200
        result['status'] = (
            'created' if result.get('relation_created')
        ...
        current_app.logger.error(f"Error upserting text for concept {concept_id}: {e}", exc_info=True)
...
def update_concept_text_relation_route(concept_id: str, relation_id: str):
    ...
    try:
        ...
        current_app.logger.error(f"Error updating text relation {relation_id} for concept {concept_id}: {e}", exc_info=True)
...
def delete_concept_text_by_predicate_route(concept_id: str):
    ...
    try:
        ...
        current_app.logger.error(f"Error deleting text relation by predicate {predicate} and text '{text}' for concept {concept_id}: {e}", exc_info=True)
...
def delete_concept_text_relation_route(concept_id: str, relation_id: str):
    ...
    try:
        ...
        current_app.logger.error(f"Error deleting text relation {relation_id} for concept {concept_id}: {e}", exc_info=True)
        return jsonify(error="Failed to delete text relation"), 500
