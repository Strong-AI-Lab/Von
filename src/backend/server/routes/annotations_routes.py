...
def annotate_turn_route():
    ...
    current_app.logger.debug(f"[annotations/turn] user={user_ctx.get('user_id')} org={user_ctx.get('org_id')} lang={user_ctx.get('language')} turn={payload.get('turn_id')}")
    ...
    llm_flag = _norm_flag(llm_flag) if llm_flag is not None else None  # None preserves auto-env behavior
    match_flag = metadata.get('match') if metadata else None
    heuristic_flag = metadata.get('heuristic') if metadata else None  # backward compatibility
    ...
    if not spans:
        ...
        try:
            ...
            try:
                if (llm_flag or (llm_flag is None)) and metadata.get('llm_enrich', True):
                    io = get_last_llm_io()
        ...
        try:
            for span in spans:
                ...
                try:
                    ...
                    current_app.logger.exception(f"Error normalizing candidates for span '{text}': {e}")
                    candidates = []
                if not isinstance(candidates, list):
                    current_app.logger.debug(f"annotations: unexpected candidates type {type(candidates)} for text={text}; coercing to list")
                    candidates = []
            ...
            current_app.logger.exception(f"Error calling concept service for suggestions: {e}")
...
def accept_annotation_route():
    ...
    current_app.logger.info(f"[annotations/accept] user={user_id} org={user_ctx.get('org_id')} turn={turn_id} concept={candidate.get('concept_id')}")
    ...
    try:
        ...
        relation = {
            'subject_concept_id': candidate.get('concept_id') or candidate.get('id') or candidate.get('conceptId'),
...
def revoke_annotation_route():
    ...
    if not (turn_id or candidate_id or object_text):
        return jsonify(error='must provide turn_id or candidate_id or object_text to revoke'), 400
    ...
    try:
        ...
        return jsonify({'status': 'revoked', 'deleted_count': result.deleted_count}), 200
    except Exception as e:
...
def manual_instance_log_route():
    ...
    if not (span_text and instance_cid and parent_type):
        return jsonify(success=False, error='span_text, instance_concept_id, parent_type_id required'), 400
    try:
...
def manual_type_log_route():
    ...
    if not (span_text and type_cid and parent_type):
        return jsonify(success=False, error='span_text, type_concept_id, parent_type_id required'), 400
    try:
        current_app.logger.info(
    ...
    try:
        ...
        return jsonify({
            ...
            'llm_io': {
                'prompt': io.get('prompt'),
                'output': out,
                'truncated': truncated or bool(io.get('truncated')),  # preserve original flag if set
                'ts': io.get('ts')
            }
        }), 200
    except Exception as e:
