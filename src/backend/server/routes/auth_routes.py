...
from backend.services.settings_service import set_current_user_by_email, get_current_user
...
def login():
    ...
    print(f"[auth_login] start state={authorization_url.split('state=')[1].split('&')[0] if 'state=' in authorization_url else 'unknown'} redirect={service.redirect_uri}")
...
def callback():
    ...
    # Condensed debug
    print(f"[auth_callback] session_state={session_state} received_state={received_state} session_keys={list(session.keys())}")
    ...
    if not valid_state:
        ...
        return f"Invalid state. Session: {session_state}, Received: {received_state}", 400

    try:
        print(f"[auth_callback] exchanging_code state={received_state}")
        id_info = service.exchange_code_for_tokens(request.url)
        print(f"[auth_callback] id_info_keys={list(id_info.keys()) if id_info else 'None'}")
        ...
        try:
            ...
            return jsonify({"error": "Multiple users found for this email", "conflicting_users": e.user_ids}), 409
        ...
        return f"An error occurred during authentication: {type(e).__name__}: {str(e)}", 500
...
def exchange_auth_token():
    ...
    if not user:
        return jsonify({"success": False, "error": "User information not found in token"}), 400
...
def get_auth_status():
    ...
    print(f"[auth_status] Host={host_hdr} Cookies={cookie_keys} Session={dict(session)}")
