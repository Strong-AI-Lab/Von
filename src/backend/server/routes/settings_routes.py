...
import tempfile
import logging
from ...vontology.utils_vontology import get_concept_notes, EXAMPLE_USER_CONCEPT_ID  # relative import
...
from ...db.mongo_client import MONGO_URI, DATABASE_NAME, get_db, get_effective_mongo_uri, is_using_fallback_uri
import re
...
def _validate_unified_concept_schema(concept, index):
    ...
    if 'names' in concept:
        if not isinstance(concept['names'], list):
            ...
            for i, name in enumerate(concept['names']):
                ...
                elif 'text' not in name and 'name' not in name:  # Allow both 'text' and 'name'
                    errors.append(f"Item {index}, names[{i}]: Missing required 'text' or 'name' field")
        ...
        if not isinstance(concept['relationships'], (list, dict)):
            ...
            for i, rel in enumerate(concept['relationships']):
                if not isinstance(rel, dict):
                    errors.append(f"Item {index}, relationships[{i}]: Must be an object")
                elif 'predicate' not in rel or 'target' not in rel:
                    errors.append(f"Item {index}, relationships[{i}]: Missing required 'predicate' or 'target' field")
    ...
    if 'concept_data' in concept:
        if not isinstance(concept['concept_data'], dict):
            ...
            if not isinstance(concept['concept_data']['preserved_fields'], dict):
                errors.append(f"Item {index}: 'concept_data.preserved_fields' must be an object")
    ...
    timestamp_fields = ['created_timestamp', 'last_modified', 'last_accessed', 'created_at', 'updated_at', 'accessed_at']
...
settings_bp = Blueprint('settings', __name__) # REMOVED url_prefix, as it's set during registration
...
def _compute_orphan_metrics(limit_sample: int = 25) -> dict:
    ...
    existing_ids = {doc.get('concept_id') for doc in all_concepts if doc.get('concept_id')}
...
def get_db_location_info():
    ...
    try:
        ...
        is_local = bool(re.search(r"mongodb://(localhost|127\.0\.0\.1|0\.0\.0\.0)", sanitized_uri, re.IGNORECASE))
        ...
        if using_fallback:
            ...
            try:
                ...
                with urllib.request.urlopen("https://api.ipify.org?format=text", timeout=1.5) as resp:
        ...
        return jsonify({
            ...
            "primary_uri_sanitized": _sanitize_mongo_uri_for_display(MONGO_URI) if using_fallback else None,
                ...
                if not entity.get("name"):
                    ...
                    if isinstance(metadata, dict) and metadata.get("title"):
                        ...
                        if isinstance(first, dict):
                            entity["name"] = first.get("text") or first.get("name") or entity.get("name")
                actual_id = entity["id"]
                current_app.logger.info(f"Found entity by concept_id {concept_id}, actual ID: {actual_id}")
                return entity, actual_id
        except Exception as e:
            current_app.logger.warning(f"Failed to get entity by concept_id {concept_id}: {e}")
...
def get_all_settings():
    ...
    try:
        ...
        org_concept_id = request.args.get("organisation_concept_id") or request.args.get("organization_concept_id")
        resolved = resolve_llm_setting(user_concept_id=user_concept_id, org_concept_id=org_concept_id)
    ...
    try:
        if 'active_llm' in data and data['active_llm']:
            ...
            if scope in ('user','organisation') and concept_id and provider and model:
                ok = set_user_llm_setting(concept_id, provider, model) if scope == 'user' else set_org_llm_setting(concept_id, provider, model)
                if ok:
                    current_app.logger.info(f"Scoped LLM override set scope={scope} concept={concept_id} provider={provider} model={model}")
            elif provider and model:
                set_active_llm_setting(provider, model)
                current_app.logger.info(f"Active LLM set to: Provider={provider}, Model={model}")
        ...
        if 'openai_api_key_env_var' in data and data['openai_api_key_env_var']:
            ...
            current_app.logger.info(f"OpenAI API key environment variable set to: {env_var}")
        ...
        return jsonify({"status": "error", "message": "An unexpected error occurred while saving settings."}), 500
...
def set_llm_override():
    ...
    try:
        ...
        if not all([provider, model, scope, concept_id]):
            return jsonify({"status": "error", "message": "provider, model, scope, concept_id required"}), 400
        if scope not in ('user','organisation'):
            return jsonify({"status": "error", "message": "scope must be 'user' or 'organisation'"}), 400
        assert isinstance(concept_id, str) and isinstance(provider, str) and isinstance(model, str)
        if scope == 'user':
            ok = set_user_llm_setting(concept_id, provider, model)
        else:
            ok = set_org_llm_setting(concept_id, provider, model)
        if not ok:
            return jsonify({"status": "error", "message": "Failed to persist override"}), 500
        resolved = resolve_llm_setting(user_concept_id=concept_id if scope=='user' else None,
...
def check_environment_variable():
    ...
    data = request.get_json()
    if not data:
        current_app.logger.warning("No JSON data received in environment variable check request")
    ...
    if value:
        ...
        current_app.logger.info(f"Environment variable {env_var_name} exists (masked: {masked_value})")
    else:
...
def get_ollama_models():
    ...
    try:
        from ...languagemodels.llm_interface import OllamaClient  # inline import to avoid circulars
        ...
        error_msg = str(e)
        current_app.logger.error(f"Error retrieving Ollama models: {error_msg}", exc_info=True)
...
def get_openai_models():
    ...
    try:
        ...
        if models:
            ...
            return jsonify({"error": "Could not retrieve OpenAI models. Please check your API key and permissions."}), 400
        ...
        error_msg = str(e)
        current_app.logger.error(f"Error retrieving OpenAI models: {error_msg}", exc_info=True)

        # Check if it's an OpenAI-specific error that should return 400 vs 500
        if "api" in error_msg.lower() or "unauthorized" in error_msg.lower() or "invalid" in error_msg.lower():
            return jsonify({"error": f"OpenAI API error: {error_msg}"}), 400
        else:
            return jsonify({"error": "An unexpected error occurred while retrieving OpenAI models."}), 500
...
def get_user_prefs(user_concept_id: str):
    ...
    try:
        ...
        current_app.logger.error(f"Error getting user prefs for {user_concept_id}: {e}", exc_info=True)
...
def set_user_prefs(user_concept_id: str):
    ...
    try:
        ...
        desired_lang = preferred_language.strip() if isinstance(preferred_language, str) else None
        ...
        for current_lang in list(existing_lang):
            ...
            try:
                ...
                current_app.logger.debug(f"preferred_language remove skipped for {user_concept_id}: missing concept", exc_info=False)
        ...
        if desired_lang:
            if desired_lang not in existing_lang:
                try:
                    ...
                    current_app.logger.warning(f"Failed to set preferred_language for {user_concept_id}")
        else:
        ...
        desired_org = organisation_concept_id.strip() if isinstance(organisation_concept_id, str) else None
        ...
        for current_org in list(existing_org):
            ...
            try:
                ...
                current_app.logger.debug(f"organisation remove skipped for {user_concept_id}: missing concept", exc_info=False)
        ...
        if desired_org:
            if desired_org not in existing_org:
                try:
                    ...
                    current_app.logger.warning(f"Failed to set organisation preference for {user_concept_id}")
        else:
        ...
        return jsonify({
            ...
            "organisation_concept_id": existing_org[0] if existing_org else None
        }), 200
    except Exception as e:
        current_app.logger.error(f"Error setting user prefs for {user_concept_id}: {e}", exc_info=True)
...
def verify_openai_api_key():
    ...
    current_app.logger.info(f"Attempting to verify OpenAI API key using environment variable: {api_key_env_var}")

    try:
        client = OpenAIClient(api_key_env_var=api_key_env_var)
        models = client.list_models()
        if models:
            current_app.logger.info(f"Successfully retrieved {len(models)} OpenAI models")
            return jsonify({"success": True, "models": models}), 200
        else:
            current_app.logger.warning("OpenAI API returned empty models list")
            return jsonify({"success": False, "error": "Could not retrieve OpenAI models. Please check your API key and permissions."}), 400
    except ValueError as e:
        error_msg = str(e)
        ...
        error_msg = str(e)
        current_app.logger.error(f"Error verifying OpenAI API key: {error_msg}", exc_info=True)
...
def get_ollama_hosts():
    ...
    try:
        ...
        error_msg = str(e)
        current_app.logger.error(f"Error retrieving Ollama hosts: {error_msg}", exc_info=True)
        return jsonify({"success": False, "error": "Failed to retrieve Ollama hosts"}), 500
...
def set_ollama_hosts():
    ...
    try:
        ...
        if not isinstance(hosts_list, list):
            return jsonify({"success": False, "error": "Hosts must be provided as a list"}), 400

        # Save hosts list
        if not set_ollama_hosts_list(hosts_list):
            return jsonify({"success": False, "error": "Failed to save hosts list"}), 500

        # Save active host if provided
        if active_host:
            if not set_active_ollama_host(active_host):
                return jsonify({"success": False, "error": "Failed to save active host"}), 500

        current_app.logger.info(f"Successfully saved {len(hosts_list)} Ollama hosts")
        return jsonify({"success": True, "message": "Ollama hosts saved successfully"}), 200
    except Exception as e:
        error_msg = str(e)
        current_app.logger.error(f"Error saving Ollama hosts: {error_msg}", exc_info=True)
...
def get_ollama_models_from_all_hosts():
    ...
    try:
        ...
        current_app.logger.info(f"Retrieved {len(all_models)} Ollama models from all hosts")
        return jsonify({"success": True, "models": all_models}), 200
    except Exception as e:
        error_msg = str(e)
        current_app.logger.error(f"Error retrieving Ollama models: {error_msg}", exc_info=True)
        return jsonify({"success": False, "error": "Failed to retrieve Ollama models"}), 500
...
def verify_ollama_host():
    ...
    try:
        ...
        if models:
            current_app.logger.info(f"Successfully connected to Ollama host {host_url} and retrieved {len(models)} models")
            return jsonify({
                "success": True,
            ...
            current_app.logger.warning(f"Connected to Ollama host {host_url} but no models found")
            return jsonify({
                "success": True,
        ...
        error_msg = str(e)
        current_app.logger.error(f"Error verifying Ollama host {host_url or ''}: {error_msg}", exc_info=True)
        return jsonify({"success": False, "error": f"Failed to connect to Ollama host: {error_msg}"}), 400
    ...
    try:
        ...
        for concept in concepts:
            ...
            people_options.append({
                ...
                "notes": (get_concept_notes(concept) or "")[:100],  # Truncate notes for display
                "concept_type": concept.get("direct_concept_name", "Person"),
                "system_tags": concept.get("system_tags", [])  # Include system_tags for filtering
            })
        ...
        current_app.logger.error(f"Error retrieving people for selection: {e}", exc_info=True)
        return jsonify({"error": "An unexpected error occurred while retrieving people."}), 500
...
def get_available_organisations():
    ...
    try:
        ...
        for concept in unique_concepts:
            from backend.vontology.utils_vontology import get_concept_display_name_with_names_fallback
            display_name = get_concept_display_name_with_names_fallback(concept)
        ...
        current_app.logger.error(f"Error retrieving organisations for selection: {e}", exc_info=True)
        return jsonify({"error": "An unexpected error occurred while retrieving organisations."}), 500
...
def export_ontology():
    ...
    try:
        ...
        for i, doc in enumerate(concepts_cursor):
            ...
            if 'concept_id' not in doc or not doc['concept_id']:
                ...
                if repaired_id:
                    ...
                    repaired_count += 1
                    issues_found.append(f"Concept {i}: Generated missing concept_id: {repaired_id}")
                    current_app.logger.warning(f"Repaired missing concept_id for document {i}: {repaired_id}")
                else:
                    issues_found.append(f"Concept {i}: Could not generate concept_id - may cause import issues")
                    current_app.logger.error(f"Could not repair concept_id for document {i}")
        ...
        # Log repair summary
        if repaired_count > 0:
            current_app.logger.warning(f"Repaired {repaired_count} concepts during export")
        if issues_found:
        ...
        try:
            json.dump(concepts_list, temp_file, indent=2, ensure_ascii=False, default=str)
            temp_file.close()
...
def repair_ontology():
    ...
    try:
        ...
        missing_id_docs = list(concepts_collection.find({
            "$or": [
        ...
        current_app.logger.info(f"Found {len(missing_id_docs)} concepts missing concept_id")
        ...
        try:
            ...
            if current_concepts:
                ...
                current_app.logger.info(f"Created backup collection: {backup_collection_name}")
        except Exception as e:
            ...
            if repaired_id:
                ...
                if result.modified_count > 0:
                    ...
                    repair_details.append({
                        ...
                        "based_on": doc.get('name') or doc.get('direct_concept_name') or 'document_id'
                    })
                    current_app.logger.info(f"Repaired concept_id for {doc['_id']}: {repaired_id}")
...
def import_ontology():
    ...
    try:
        ...
        gate_env = os.getenv("VON_ALLOW_IMPORT") or os.getenv("VON_ENABLE_IMPORT")  # allow either name
        admin_header = request.headers.get("X-Von-Admin-Token") or request.headers.get("X-Admin-Token")
        required_token = os.getenv("VON_ADMIN_TOKEN")

        if not gate_env:
            return jsonify({"error": "Ontology import disabled (set VON_ALLOW_IMPORT=1)"}), 403
        if required_token:
        ...
        destructive = (req_mode == 'replace') or (not incremental and not replace_all_flag) and not dry_run_flag  # legacy default remains destructive apply unless incremental explicitly requested
        ...
        if not isinstance(import_data, list):
            return jsonify({"error": "JSON file must contain an array of concepts"}), 400
        ...
        if validation_errors:
            current_app.logger.error(f"Validation errors: {validation_errors[:10]}")  # Log first 10 errors
            error_summary = f"Validation failed. Found {len(validation_errors)} error(s)."
        ...
        try:
            ...
            current_app.logger.warning(f"Failed to enumerate existing concept ids for diff: {e}")
        ...
        if incremental and unchanged:
            ...
            existing_map = {}
            try:
                for doc in concepts_collection.find({"concept_id": {"$in": list(unchanged)}}, {"_id":0}):
                    cid = doc.get('concept_id')
                    if cid:
                        existing_map[cid] = doc
            except Exception as _e:  # pragma: no cover - defensive
                current_app.logger.warning(f"[import] incremental existing fetch failed: {_e}")
            for c in valid_concepts:
                cid = c.get('concept_id')
                ...
                if changed_fields:
                    ...
                    if len(updates_detail) < 25:
                        updates_detail.append({"concept_id": cid, "changed_fields": changed_fields})
        ...
        if dry_run_flag:
            ...
            try:
                ...
                if incremental:
                    _IMPORT_METRICS['incremental_run_count'] = _IMPORT_METRICS.get('incremental_run_count',0) + 1
            except Exception as _e:  # pragma: no cover - defensive
                current_app.logger.warning(f"[import] metrics update failed (dry-run): {_e}")
            payload = {
                "success": True,
                "mode": mode,
                "diff": diff_stats,
        ...
        if incremental:
            ...
            if to_add:
                ...
                try:
                    ...
                    current_app.logger.warning("[import] incremental insert partial failures: %s", len(bwe.details.get('writeErrors', [])))
            # Apply updates (shallow)
            if update_ids:
                existing_map2 = {}
                try:
                    for doc in concepts_collection.find({"concept_id": {"$in": list(update_ids)}}, {"_id":0}):
                        cid = doc.get('concept_id')
                        if cid:
                            existing_map2[cid] = doc
                except Exception as _e:  # pragma: no cover
                    current_app.logger.warning(f"[import] incremental update fetch failed: {_e}")
                for c in valid_concepts:
                    cid = c.get('concept_id')
                    if cid in update_ids:
                        ...
                        if patch:
                            try:
                                concepts_collection.update_one({"concept_id": cid}, {"$set": patch})
                                updated_count += 1
                            except Exception as _e:  # pragma: no cover
                                current_app.logger.warning(f"[import] incremental update failed for {cid}: {_e}")
            current_app.logger.info(f"[import] incremental complete: added={added_count} updated={updated_count} unchanged={len(unchanged) - len(update_ids)}")
            resp = {
                "success": True,
                "mode": mode,
            ...
            try:
                ...
                current_app.logger.warning(f"[import] metrics update failed (incremental apply): {_e}")
        ...
        current_app.logger.info(
            ...
            existing_count, len(valid_concepts), len(to_add), len(to_delete), len(unchanged)
        )
        ...
        try:
            ...
            if coll2 is None:
                current_app.logger.error("Could not access concepts collection after drop")
            ...
            current_app.logger.info(f"[import] apply complete: {inserted_count} inserted")
            resp = {
                "success": True,
                "mode": mode,
            ...
            try:
                ...
                current_app.logger.warning(f"[import] metrics update failed (apply): {_e}")
            return jsonify(resp), 200
        except Exception as e:
...
def get_deprecation_metrics():
    ...
    try:
        ...
        fe_total_ms = fe_totals.get('total_ms') if isinstance(fe_totals.get('total_ms'), (int, float)) else None
