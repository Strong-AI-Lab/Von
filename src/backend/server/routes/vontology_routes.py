...
from ...services.concept_service import get_concept_by_id, get_concept_by_concept_id, ConceptNotFoundError, update_concept_description
from ...services.text_value_service import get_texts_for_concept
from ...services.concept_search_service import search_concepts as search_concepts_service
...
def record_name_normalization_route():
    ...
    try:
        ...
        current_app.logger.debug("Failed to record name normalization metric", exc_info=True)
...
def record_frontend_tree_load_route():
    ...
    try:
        if load_ms is not None:
            record_frontend_tree_load(int(load_ms), node_count=node_count, cache_hit=cache_hit)
        else:
...
def get_tree():
    ...
    if cache_record:
        ...
        with _TREE_CACHE_LOCK:
            try:
                _TREE_CACHE[cache_key] = (cache_record[0], payload, build_secs, alloc_kb, hits + 1)
                hits += 1
            except Exception:
                pass
        current_app.logger.debug(
            "Tree cache hit (age=%.1fs build=%.2fs alloc=%.1fKB size_nodes=%s)",
            now - cache_record[0], build_secs, alloc_kb, len(payload.get('nodes', [])) if isinstance(payload, dict) else 'n/a'
        )
        return jsonify({**payload, '_cache': {'hit': True, 'age_sec': now - cache_record[0], 'build_sec': build_secs, 'alloc_kb': alloc_kb, 'ttl_sec': _TREE_CACHE_TTL_SECONDS, 'hits': hits}})
    # Build fresh
    current_app.logger.debug("Tree cache miss (refresh=%s); building...", force_refresh)
    t0 = time.time(); phase_marks = []; phase_log_enabled = (os.getenv('VON_TREE_PHASE_LOG') in ('1','true','TRUE','True')) and (not getattr(current_app, 'testing', False))
    def _mark(phase_name):
        if not phase_log_enabled:
            return
        now = time.time(); phase_marks.append((phase_name, now))
    ...
    try:
        ...
        if "error" in tree_data:
            current_app.logger.error("Error getting Vontology tree: %s", tree_data['error'])
            return jsonify({"error": tree_data["error"]}), 500
    except Exception as e:
        current_app.logger.error("Unexpected error building Vontology tree: %s", e, exc_info=True)
        return jsonify({"error": "Failed to retrieve Vontology structure due to an internal server error."}), 500
    _mark('tree_built')
    ...
    try:
        if os.getenv('VON_TRACE_TREE') in ('1','true','TRUE','True') and stats:
            ...
            sorted_stats = sorted(stats, key=lambda s: abs(s.size_diff), reverse=True)[:top_n]
            for s in sorted_stats:
                if s.size_diff == 0:
                    continue
                current_app.logger.info(
                    "[tree_alloc_diff] %s size_diff=%.1fKB count_diff=%d", s.traceback[0].filename if s.traceback else 'unknown', s.size_diff/1024.0, s.count_diff
                )
    except Exception:
        pass
    ...
    try:
        ...
        pass
    current_app.logger.info("Built Vontology tree build=%.2fs alloc_diff=%.1fKB nodes=%s", build_secs, alloc_kb, len(tree_data.get('nodes', [])) if isinstance(tree_data, dict) else 'n/a')
    if phase_log_enabled:
        ...
        try:
            ...
            current_app.logger.info("[tree_build_phase] %s total=%.1fms", ' '.join(phases_out), total_ms)
        except Exception:
            pass
        ...
        try:
            if os.getenv('VON_TREE_PHASE_LOG') in ('1','true','TRUE','True'):
                ...
                try:
                    ...
                    pass
                logger.info("[tree_build_async] total=%.1fms nodes=%s", total_ms, node_count)
        except Exception:
            pass
...
def tree_build_progress(job_id: str):
    ...
    response = {'job_id': job_id, 'progress': progress, 'done': bool(status.get('result') or status.get('error'))}
...
def unified_delete_node():
    ...
    if not concept_id:
        resp = {"success": False, "error": "concept_id required", "simulate": True, "correlation_id": correlation_id}
        r = jsonify(resp)
    ...
    response_payload = {
        ...
        'children_reparented': result.get('children_reparented', len(result.get('children', []))),
        'children': result.get('children', []),
        'instances_retyped': result.get('instances_retyped', len(result.get('instances', []))),
    ...
    if response_payload.get('success') and not response_payload.get('simulate'):
        try:
            ...
            affected = [concept_id] + [c.get('child_id') for c in response_payload['children']]
...
def list_text_relations():
    ...
    try:
        ...
        current_app.logger.error('Failed to list text relations for %s: %s', concept_id, e, exc_info=True)
...
def get_node_content_route():
    ...
    try:
        ...
        current_app.logger.error(f"Error getting node content for '{identifier}': {e}", exc_info=True)
        return jsonify({"error": "Failed to retrieve node content due to an internal server error."}), 500
...
def get_parents_route():
    ...
    try:
        ...
        try:
            if ObjectId and hasattr(ObjectId, 'is_valid') and ObjectId.is_valid(identifier):
                try:
        ...
        current_app.logger.error(f"Unexpected error fetching parents for node '{identifier}': {e}", exc_info=True)
        return jsonify({"error": "Failed to retrieve parents due to an internal server error."}), 500
...
def update_description_route():
    ...
    current_app.logger.info(f"Received request for /api/vontology/update_description with identifier: '{identifier}'")

    if not identifier or description is None:
        current_app.logger.warning("Missing 'identifier' or 'description' in /update_description request.")
        return jsonify({"success": False, "error": "Missing 'identifier' or 'description' parameter."}), 400
    ...
    try:
        ...
        try:
            ...
            return jsonify({"success": False, "error": f"Node with identifier '{identifier}' not found."}), 404
        except Exception:  # pragma: no cover
            current_app.logger.exception("Error resolving identifier to concept_id in update_description_route; using original")

        ok = update_concept_description(resolved_concept_id, description)
        if not ok:
            return jsonify({"success": False, "error": f"Failed to update description for '{resolved_concept_id}'."}), 400
        return jsonify({"success": True, "concept_id": resolved_concept_id}), 200
    except Exception as e:
        current_app.logger.error(f"Unexpected error updating description for node '{identifier}': {e}", exc_info=True)
        return jsonify({"success": False, "error": "Failed to update description due to an internal server error."}), 500
...
def create_concept_route():
    ...
    try:
        ...
        current_app.logger.error(f"Unexpected error creating Vontology concept '{new_concept_name}' under '{parent_id}': {e}", exc_info=True)
...
def add_closure_route():
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Upward closure nodes added successfully."}), 200
    except Exception as e:
        current_app.logger.error(f"Unexpected error adding closure for node '{node_path}': {e}", exc_info=True)
        return jsonify({"success": False, "message": "An internal server error occurred while adding closure nodes."}), 500
...
def import_nodes_route():
    ...
    if is_opencyc_format(data):
        ...
        try:
            ...
            current_app.logger.info(f"Converted {len(nodes_to_import)} OpenCyc nodes to Von format.")
        except Exception as e:
            current_app.logger.error(f"Error converting OpenCyc format: {e}")
            return jsonify({"success": False, "message": f"Error converting OpenCyc format: {str(e)}"}), 400

    # Check for legacy upward closure format (backup for old data)
    elif isinstance(data, dict) and "edges" in data and "nodes" in data and not is_opencyc_format(data):
        current_app.logger.info("Detected legacy upward closure format, processing with custom logic.")
        ...
        for node_data in raw_nodes:
            ...
            label = node_data.get('label')
            if not source_id or not label:
                current_app.logger.warning(f"Skipping node with missing id or label in upward closure data: {node_data}")
                continue
    ...
    # Check for standard format (payload is a list of nodes)
        ...
        current_app.logger.info("Processing as standard format (payload is a list of nodes).")
    ...
    # Handle standard format wrapped in a 'nodes' key
        ...
        current_app.logger.info("Processing as standard format (payload is an object with a 'nodes' array).")
        nodes_to_import = data['nodes']

    else:
        current_app.logger.warning("Missing or invalid 'nodes' array in /import_nodes request.")
        return jsonify({"success": False, "message": "Request payload must be a JSON array of nodes, or an object containing a 'nodes' array."}), 400

    if not isinstance(nodes_to_import, list):
        current_app.logger.warning(f"Processed nodes are not a list, but {type(nodes_to_import)}.")
        return jsonify({"success": False, "message": "Internal error processing nodes."}), 500

    if len(nodes_to_import) == 0:
        return jsonify({"success": True, "message": "No new nodes to import.", "imported_count": 0}), 200
    ...
    try:
        ...
        try:
            safe_cycles_flag = (request.args.get('safe_cycles') or '').lower() in ('1','true','yes')
        ...
        if isinstance(result, dict) and 'error' in result:
            current_app.logger.error(f"Error importing ontology nodes: {result['error']}")
        ...
        if safe_cycles_flag:
            ...
            enhanced_result.setdefault('cycle_info', {'has_cycles': False, 'strategy': 'none', 'applied': False})
        return jsonify(enhanced_result), 200
    except Exception as e:
        current_app.logger.error(f"Unexpected error importing ontology nodes: {e}", exc_info=True)
        return jsonify({'success': False, 'message': 'An internal server error occurred while importing nodes.'}), 500
...
def list_import_progress_route():
    store = current_app.config.get('VONTOLOGY_IMPORT_PROGRESS', {})
    summary = {jid: {k: v for k, v in d.items() if k in ('processed','total','imported','updated','skipped','status')} for jid, d in store.items()}
...
def import_preview_route():
    ...
    if request.files and 'file' in request.files:
        ...
        data = request.get_json()
        if not data:
            current_app.logger.warning("Received empty JSON payload for /import_preview.")
            return jsonify({"success": False, "message": "Request body must be JSON."}), 400
    ...
    try:
        ...
        if is_opencyc_format(data):
            ...
            try:
                ...
                current_app.logger.info(f"Converted {len(nodes_to_preview)} OpenCyc nodes for preview.")
            except Exception as e:
                current_app.logger.error(f"Error converting OpenCyc format for preview: {e}")
                return jsonify({"success": False, "message": f"Error converting OpenCyc format: {str(e)}"}), 400
...
def analyze_import_preview(nodes_to_import):
    ...
    from ...vontology.utils_vontology import detect_circular_references_in_import, break_cycles_in_import_nodes
    ...
    try:
        ...
        if cycle_check["has_cycles"]:
            current_app.logger.warning(f"Preview detected {len(cycle_check['cycles'])} circular references (strict_mode={strict_env})")
            for cycle in cycle_check["cycles"]:
                cycle_str = " → ".join(cycle)
                cycle_meta["warnings"].append(f"Circular reference detected: {cycle_str}")
            if not strict_env:
                ...
                if breaker.get('unresolved_cycle_count', 0) > 0:
                    cycle_meta["warnings"].append(f"Unresolved cycles remain: {breaker.get('unresolved_cycle_count')}")
            else:
        ...
        current_app.logger.error(f"Error during cycle detection/breaking in preview: {e}")
        analysis["circular_references"]["warnings"].append(f"Could not perform cycle analysis: {str(e)}")
    ...
    ACRONYMS = {"AI","NLP","LLM","GPU","CPU","API","HTTP","HTTPS","JSON","SQL","ID","UUID","URL","UI","UX","ML","RL","DL"}
    ...
    def _humanize_concept_id(cid: str) -> str:
        ...
        return " ".join(_normalize_word(w.upper() if w.upper() in ACRONYMS else w) for w in words)
    ...
    def derive_display_name(node: dict) -> str:
        ...
        if label:
            return _humanize_concept_id(label)  # label might already be fine; still normalize acronyms/case
    ...
    try:
        if nodes_to_import and all(isinstance(n, dict) for n in nodes_to_import):
            ...
            elif any('label' in n and 'comment' in n for n in nodes_to_import) and not any('concept_id' in n for n in nodes_to_import):
                analysis['format_detected'] = 'legacy_closure'
            elif any('names' in n for n in nodes_to_import):
                analysis['format_detected'] = 'von_extended'
            elif any(n.get('concept_id','').startswith('#V#') for n in nodes_to_import):
...
def export_nodes_route():
    ...
    try:
        ...
        current_app.logger.error(f"Unexpected error exporting ontology nodes: {e}", exc_info=True)
        return jsonify({"error": "An internal server error occurred while exporting nodes."}), 500
...
def search_concepts():
    ...
    include_individuals = (request.args.get('include_individuals') or '').lower() in ('1', 'true', 'yes', 'y')
    # Allow disabling fallback via query param; default on
    fallback_substring = (request.args.get('fallback_substring') or 'true').lower() in ('1', 'true', 'yes', 'y')
    ...
    try:
        ...
        current_app.logger.error(f"Error performing Vontology search: {e}", exc_info=True)
        return jsonify({"error": "Failed to perform search due to an internal server error."}), 500
    ...
    try:
        ...
        for concept_id in test_concepts:
            try:
                ...
                full_concept_doc = ConceptsRepository.find_one({"concept_id": concept_id})
                ...
                if concept_data and full_concept_doc:
                    ...
                    results.append({
                        "concept_id": concept_id,
                        "found": True,
                        "name": get_concept_display_name_with_names_fallback(concept_data[0]) if concept_data else "Unknown",
                        "path": concept_data[0].get("path") if concept_data else "Unknown",
...
def debug_database():
    ...
    try:
        ...
        research_concepts = list(repo.find(
            {"$or": [
...
def delete_node():
    ...
    concept_id = data.get("concept_id")
    if not concept_id:
        return jsonify({"success": False, "message": "Missing 'concept_id' parameter."}), 400
    ...
    try:
        ...
        if not existing_node:
            return jsonify({"success": False, "message": f"Node with concept_id '{concept_id}' not found."}), 404
        ...
        if result.deleted_count > 0:
            ...
            return jsonify({"success": False, "message": f"Failed to delete node '{concept_id}'."}), 500

    except Exception as e:
        current_app.logger.error(f"Error deleting node {concept_id}: {e}", exc_info=True)
...
def get_entity_counts():
    ...
    try:
        ...
        vontology_concepts = list(repo.find({
            "concept_id": {"$regex": "^#V#"}
        }, {'concept_id': 1, 'name': 1}))
        ...
        current_app.logger.debug(f"Generated entity counts for {len(entity_counts)} concepts")
        # Update lightweight stats
        try:
            duration_ms = int((time.time() - start_time) * 1000)
            _ENTITY_COUNTS_STATS['total_calls'] = int(_ENTITY_COUNTS_STATS.get('total_calls', 0)) + 1
            _ENTITY_COUNTS_STATS['last_ts'] = int(time.time())
            prev = _ENTITY_COUNTS_STATS.get('ema_ms')
            alpha = 0.2
            _ENTITY_COUNTS_STATS['ema_ms'] = duration_ms if prev in (None, 0) else (alpha * duration_ms + (1 - alpha) * float(prev))
        except Exception:
            pass
    ...
    return jsonify({"error": "Failed to retrieve entity counts due to an internal server error."}), 500
    ...
    try:
        ...
        if cached:
            ...
            if now_ts - ts < _INSTANCE_COUNTS_CACHE_TTL_SECONDS:
                current_app.logger.debug(f"/instance_counts cache hit for key: {cache_key}")
        ...
        for tid in type_ids:
            try:
                ...
                try:
                        ...
                        per_desc_query = {
                            "$and": [
                                {"$or": [
                                    ...
                                    {"relationships.is_an_instance_of": {"$in": [did]}}
                                ]},
                                {"$or": [
                                    {"metadata.concept_type": {"$exists": False}},
                                    {"metadata.concept_type": {"$ne": "collection"}}
                    ...
                    indirect_query = {"relationships.is_an_instance_of": {"$in": descendant_ids}}
                ...
                current_app.logger.error(f"Error computing counts for {tid}: {e}", exc_info=True)
        ...
        try:
            ...
            current_app.logger.debug("Failed to write to instance_counts cache; continuing without caching")

        return jsonify({"instance_counts": results})
    except Exception as e:
        current_app.logger.error(f"Error in /instance_counts: {e}", exc_info=True)
        return jsonify({"error": "Failed to compute instance counts due to an internal server error."}), 500
...
def get_node_children():
    ...
    current_app.logger.info(f"Received request for /api/vontology/children with node_id: '{node_id}'")
    ...
    try:
        ...
        for doc in children_cursor:
            ...
            resolved_name = get_concept_display_name_with_names_fallback(doc) or doc.get("name", "") or (doc.get("concept_id") or "")
                    ...
                    if isinstance(names, list):
                        for entry in names:
                            ...
                            if entry.get("type") == "NL":
                                ...
                                if nm == resolved_name and (lang in (None, "", "en-NZ")):
                                    has_matching_nl = True
                                    break
                    # Only persist if we actually derived a human name and it's not already present
                    if (not has_matching_nl) and isinstance(resolved_name, str) and resolved_name and not resolved_name.startswith('#V#') and resolved_name != 'Unnamed Concept':
                        new_entry = {"name": resolved_name, "language": "en-NZ", "type": "NL"}
                        if isinstance(names, list) and len(names) > 0 and not has_any_nl:
                            # Append only when there's no NL yet
                            repo.update_one({"concept_id": cid}, {"$push": {"names": new_entry}})
                        elif not isinstance(names, list) or len(names) == 0:
                            repo.update_one({"concept_id": cid}, {"$set": {"names": [new_entry]}})
            except Exception:
                current_app.logger.debug("Failed to persist derived NL name for child %s", doc.get("concept_id"), exc_info=True)
        ...
        current_app.logger.error(f"Error getting children for node {node_id}: {e}", exc_info=True)
        return jsonify({"error": "Failed to retrieve children due to an internal server error."}), 500

@vontology_bp.route('/instances', methods=['GET'])
def get_node_instances():
    """Endpoint to fetch instances of a specific concept node."""
    node_id = request.args.get('node_id')
    include_subtypes = request.args.get('include_subtypes', '').lower() in ('1', 'true', 'yes', 'y')
    current_app.logger.info(f"Received request for /api/vontology/instances with node_id: '{node_id}'")
    ...
    try:
        ...
        try:
            ...
            current_app.logger.warning("Failed to resolve descendant type ids for include_subtypes; defaulting to direct only", exc_info=True)
        ...
        for doc in instances_cursor:
            ...
            try:
                ...
                current_app.logger.debug("Failed to persist derived NL name for instance %s", doc.get("concept_id"), exc_info=True)
        ...
        current_app.logger.debug(f"Found {len(instances)} instances for concept {node_id}")
        return jsonify({"instances": instances, "count": len(instances)})

    except Exception as e:
        current_app.logger.error(f"Error getting instances for concept {node_id}: {e}", exc_info=True)
        return jsonify({"error": "Failed to retrieve instances due to an internal server error."}), 500
...
def get_relationships_route():
    ...
    """
    identifier = request.args.get('identifier')
    current_app.logger.info(f"Received request for /api/vontology/relationships with identifier: '{identifier}'")

    if not identifier:
        return jsonify({"success": False, "error": "Missing 'identifier' parameter."}), 400
    ...
    try:
        ...
        if not doc:
            return jsonify({"success": False, "error": f"Concept '{identifier}' not found."}), 404
            ...
            if concept_identifier:
                ...
                if not existing_subtypes:
                    ...
                    for child_doc in child_cursor:
                        ...
                        if isinstance(child_id, str) and child_id and child_id not in existing_subtypes:
                            derived_children.append(child_id)
                    if derived_children:
                        out["has_subtype"] = sorted(set(out.get("has_subtype", [])) | set(derived_children))
        ...
        dynamic_raw = {k: _norm_list(v) for k, v in rel.items() if k not in out and k != 'most_salient_type'}
        ...
        if not most_salient_type:
            ...
            try:
                ...
                current_app.logger.debug(f"Computed dynamic most_salient_type for {doc.get('concept_id')}: {most_salient_type}")
            except Exception as e:
                current_app.logger.warning(f"Failed to compute dynamic most_salient_type for {doc.get('concept_id')}: {e}")
        ...
        ids_to_resolve = sorted({cid for arr in list(out.values()) + list(dynamic_raw.values()) for cid in arr})
        metadata_map = {}
        if ids_to_resolve:
            ...
            if concepts_needing_text_relations:
                ...
                text_relations = list(TextRelationsRepository.find({
                    "subject_concept_id": {"$in": concepts_needing_text_relations},
                    "predicate": "hasName"
                }))
                ...
                if text_relations:
                    ...
                    text_values = list(TextValuesRepository.find({
                        "_id": {"$in": [ObjectId(tid) if ObjectId else tid for tid in text_value_ids]}
                    }))

                    # Build text_id -> text map
                    text_map = {str(tv["_id"]): tv.get("text", "") for tv in text_values}
        ...
        for k, arr in out.items():
            ...
            for cid in arr:
                ...
                if k == "is_a_type_of" and most_salient_type and cid == most_salient_type:
        ...
        for k, arr in dynamic_raw.items():
            enriched[k] = [
                {
                    "id": cid,
                    "name": metadata_map.get(cid, {"name": cid, "kind": "individual"})["name"],
                    "kind": metadata_map.get(cid, {"name": cid, "kind": "individual"})["kind"]
                }
                for cid in arr
            ]
        ...
        current_app.logger.error(f"Error getting relationships for '{identifier}': {e}", exc_info=True)
        return jsonify({"success": False, "error": "Failed to retrieve relationships due to an internal server error."}), 500
...
def add_relationship_route():
    ...
    current_app.logger.info(f"Received request for /api/vontology/relationships/add: {data}")

    if not source_id or not target_id or not kind:
        return jsonify({"success": False, "error": "Missing 'source_id', 'target_id', or 'kind'."}), 400

    if source_id == target_id:
        return jsonify({"success": False, "error": "Source and target cannot be the same."}), 400

    allowed = {"is_a_type_of", "has_subtype", "is_an_instance_of", "has_instance", "related_to"}
    ...
    if kind not in allowed:
        ...
        if isinstance(kind, str) and kind.startswith('#V#'):
            ...
            return jsonify({"success": False, "error": f"Invalid relationship kind '{kind}'."}), 400
    ...
    try:
        ...
        if is_dynamic_predicate:
            predicate_doc = repo.find_one({"concept_id": kind}, {"relationships.is_an_instance_of": 1})
            if predicate_doc:
                instance_of = predicate_doc.get('relationships', {}).get('is_an_instance_of', [])
                if isinstance(instance_of, str):
                    instance_of = [instance_of]
        ...
        if not src:
            return jsonify({"success": False, "error": f"Source '{source_id}' not found."}), 404
        ...
        if is_text_predicate:
            try:
                ...
                current_app.logger.info(f"Redirected binary text predicate {kind} to text relations API: {result}")
                return jsonify({"success": True, "redirected_to_text_relations": True, "result": result}), 200
            except Exception as e:
                current_app.logger.error(f"Error adding text relation for binary text predicate: {e}", exc_info=True)
                return jsonify({"success": False, "error": f"Failed to add text relation: {str(e)}"}), 500

        # For non-text predicates, ensure target concept exists
        tgt = repo.find_one({"concept_id": target_id})
        if not tgt:
            return jsonify({"success": False, "error": f"Target '{target_id}' not found."}), 404
        ...
        def _ensure_array_and_add(cid: str, rel_kind: str, rel_target: str):
            # Read current value
            existing = repo.find_one({"concept_id": cid}, {f"relationships.{rel_kind}": 1}) or {}
            ...
            if isinstance(curr, list):
                ...
                repo.update_one({"concept_id": cid}, {"$set": {f"relationships.{rel_kind}": [curr]}})
            else:
                # Initialize as empty array
                repo.update_one({"concept_id": cid}, {"$set": {f"relationships.{rel_kind}": []}})
            # Now safe to add
            repo.update_one({"concept_id": cid}, {"$addToSet": {f"relationships.{rel_kind}": rel_target}})
        ...
        return jsonify({"success": False, "error": f"Failed to add relationship: {str(e)}"}), 500
...
def list_elicitation_predicates():
    ...
    try:
        repo = ConceptsRepository
        predicate_types = ['#V#knowledge_elicitation_predicate', '#V#binary_text_predicate']
        cursor = repo.find(
            {"relationships.is_an_instance_of": {"$in": predicate_types}},
            {"concept_id": 1, "name": 1, "names": 1, "relationships.is_an_instance_of": 1}
        )
        from ...vontology.utils_vontology import get_concept_display_name_with_names_fallback
        preds = []
        for d in cursor:
        ...
        current_app.logger.error(f"Error listing elicitation predicates: {e}", exc_info=True)
        return jsonify({"success": False, "error": "Failed to list elicitation predicates."}), 500
...
def list_salient_predicates_for_instance():
    ...
    try:
        ...
        if isinstance(direct_types, str):
            ...
            direct_types = []
        if not direct_types:
            return jsonify({"success": True, "predicates": [], "types_considered": []}), 200

        # Ensure all entries are strings
        direct_types = [t for t in direct_types if isinstance(t, str) and t]

        # DEBUG: Log what direct types we found
        current_app.logger.info(f"[SALIENT_DEBUG] instance_id={instance_id} direct_types={direct_types}")
        ...
        if cached and (now - cached[0]) < CACHE_TTL_SECONDS:
            ...
            current_app.logger.info(f"/predicates/salient cache HIT instance={instance_id} key={cache_key} duration_ms={duration_ms} types={len(cached[2])} preds={len(cached[1])}")
            debug_flag = request.args.get('debug') == 'true'
            scope_payload = cached[3] if len(cached) >= 4 else None
            payload = {"success": True, "predicates": cached[1], "types_considered": cached[2], "cache": True}
            if split_enabled:
                scope_payload = scope_payload or {}
                payload['predicates_by_scope'] = scope_payload.get('predicates_by_scope', {})
                payload['predicate_origins'] = scope_payload.get('predicate_origins', {})
            if debug_flag:
                payload['metrics'] = {
        ...
        if direct_types and not force_bfs:
            type_docs = list(repo.find({"concept_id": {"$in": [t for t in direct_types if isinstance(t, str)]}}, {
                "concept_id": 1,
                "inherited_salient_binary_predicates": 1
            }))
            doc_map = {d.get("concept_id"): d for d in type_docs if isinstance(d.get("concept_id"), str)}
            all_have = True
            ...
            if all_have:
                ...
                current_app.logger.info(f"[SALIENT_DEBUG] Fast path SUCCESS for {instance_id}: predicates={fast_predicates}")
            else:
                current_app.logger.info(f"[SALIENT_DEBUG] Fast path FAILED for {instance_id}: all_have={all_have}, ordered_union_ids={ordered_union_ids}, type_docs={[d.get('concept_id') for d in type_docs]}")
        ...
        def _collect_bfs_salience(seed_types: list[str]) -> tuple[list[str], list[str]]:
            ...
            while queue and expansions < max_expansions:
                ...
                expansions += 1
                doc = repo.find_one({"concept_id": t}, {"concept_id": 1, "relationships": 1, SALIENT_SCOPE_FIELD: 1, "salient_binary_predicates_for_type": 1}) or {}
        ...
        from ...vontology.utils_vontology import get_concept_display_name_with_names_fallback
        predicates_out = []
        resolved_ids = fast_predicates if (precomputed_mode and fast_predicates) else salient_ids
        ...
        text_predicate_map = {}
        if lookup_ids:
            cursor = repo.find({"concept_id": {"$in": list(lookup_ids)}}, {"concept_id": 1, "name": 1, "names": 1, "relationships.is_an_instance_of": 1})
            for d in cursor:
                cid = d.get('concept_id')
                if not cid:
                    continue
        ...
        if split_enabled:
            ...
            for scope_name in ('instance', 'type', 'unclassified'):
                ...
                if not scope_ids:
                    ...
                    continue
                sorted_ids = sorted(scope_ids, key=lambda pid: (name_map.get(pid, pid).lower(), pid))
                scope_items = []
            ...
            predicate_origins = {
                scope_name: {pid: sorted(list(types)) for pid, types in scope_origins[scope_name].items()}
        ...
        if precomputed_mode:
            ...
            _SALIENT_METRICS['ema_fast_ms'] = duration_ms if prev is None else (alpha * duration_ms + (1-alpha) * prev)
        else:
            _SALIENT_METRICS['bfs_calls'] += 1
            _SALIENT_METRICS['bfs_predicates_total'] += len(predicates_out)
            prev = _SALIENT_METRICS.get('ema_bfs_ms')
            alpha = _SALIENT_METRICS['ema_alpha']
            _SALIENT_METRICS['ema_bfs_ms'] = duration_ms if prev is None else (alpha * duration_ms + (1-alpha) * prev)

        current_app.logger.info(f"/predicates/salient cache MISS instance={instance_id} key={cache_key} mode={mode} duration_ms={duration_ms} types={len(ordered_types_final)} preds={len(predicates_out)}")
        debug_flag = request.args.get('debug') == 'true'
        payload = {"success": True, "predicates": predicates_out, "types_considered": ordered_types_final, "cache": False, "mode": mode}
        if split_enabled:
            payload['predicates_by_scope'] = (scope_cache_entry or {}).get('predicates_by_scope', {})
            payload['predicate_origins'] = (scope_cache_entry or {}).get('predicate_origins', {})
            payload['raw_scope_map'] = (scope_cache_entry or {}).get('raw_scope_map', {})
        if debug_flag:
            payload['metrics'] = {
                'stats': _SALIENT_STATS,
                'counters': _SALIENT_METRICS,
        ...
        current_app.logger.error(f"Error listing salient predicates: {e}", exc_info=True)
        return jsonify({"success": False, "error": "Failed to list salient predicates."}), 500
...
def invalidate_salient_cache():
    ...
    current_app.logger.info(f"/predicates/salient/cache/invalidate cleared={prev_entries} stats={stats}")
...
def recompute_inherited_salient():
    ...
    current_app.logger.info(f"/predicates/salient/recompute_inherited request params force={force} limit={limit_param} raw_dry_run={raw_dry} parsed_dry_run={dry_run}")
    ...
    if isinstance(limit_param, (int, float)):
        ...
        try:
            ...
            current_app.logger.warning("Ignoring non-integer limit parameter: %s", limit_param)

    start = time.time()
    try:
        summary = recompute_salient_predicates(force=force, limit=limit_value, dry_run=dry_run)
    except Exception as exc:
        duration_ms = int((time.time() - start) * 1000)
        current_app.logger.error("recompute_inherited_salient failed force=%s dry_run=%s limit=%s error=%s", force, dry_run, limit_value, exc, exc_info=True)
        return jsonify({
            "success": False,
            "error": str(exc),
    ...
    current_app.logger.info(f"/predicates/salient/recompute_inherited force={force} dry_run={dry_run} limit={limit_param} summary={summary}")
...
def remove_relationship_route():
    ...
    kind = data.get('kind')
    target_id = data.get('target_id')
    current_app.logger.info(f"Received request for /api/vontology/relationships/remove: {data}")
    ...
    structural_allowed = {"is_a_type_of", "has_subtype", "is_an_instance_of", "has_instance", "related_to"}
    is_dynamic_predicate_kind = kind.startswith('#V#') and kind not in structural_allowed
    ...
    try:
        ...
        if is_text_predicate:
            try:
                from ...services.text_value_service import delete_text_relation_by_predicate_and_text
                result = delete_text_relation_by_predicate_and_text(
                    subject_concept_id=source_id,
                    predicate=kind,
                    text=target_id  # target_id is actually the text value for text predicates
                )
                current_app.logger.info(f"Redirected binary text predicate {kind} removal to text relations API: {result}")
                return jsonify({"success": True, "redirected_to_text_relations": True, "result": result}), 200
            except Exception as e:
                current_app.logger.warning(f"Text relation removal failed, checking concept relationships for legacy data: {e}")
                ...
                try:
                    ...
                    if src and kind in src.get('relationships', {}):
                        ...
                        if target_id in relationships.get(kind, []):
                            ...
                            current_app.logger.info(f"Removed legacy binary text predicate {kind} from concept relationships")
                            return jsonify({"success": True, "legacy_cleanup": True, "message": "Removed from concept relationships (legacy data)"}), 200

                    # Neither location had the relationship
                    return jsonify({"success": False, "error": f"Failed to remove text relation: {str(e)}"}), 500
                except Exception as fallback_e:
                    current_app.logger.error(f"Fallback removal also failed: {fallback_e}", exc_info=True)
                    return jsonify({"success": False, "error": f"Failed to remove relationship from both text relations and concept relationships: {str(e)}"}), 500
        ...
        def _ensure_array_and_remove(cid: str, rel_kind: str, rel_target: str):
            ...
            if isinstance(curr, list):
                ...
                repo.update_one({"concept_id": cid}, {"$set": {f"relationships.{rel_kind}": []}})
            repo.update_one({"concept_id": cid}, {"$pull": {f"relationships.{rel_kind}": rel_target}})
        ...
        return jsonify({"success": False, "error": f"Failed to remove relationship: {str(e)}"}), 500
...
def toggle_concept_flag():
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"Failed to toggle flag: {str(e)}"}), 500
...
def toggle_organization_relation():
    ...
    try:
        ...
        organisation_concept_id = (
            data.get('organisation_concept_id') or
            data.get('organization_concept_id') or
            data.get('organisation_id') or
        ...
        if action == 'add':
            if not organisation_concept_id:
                return jsonify({"success": False, "error": "organisation_concept_id required for add", "action": action}), 400
        ...
        ConceptsRepository.update_one({"concept_id": concept_id}, {"$set": {"relationships": relationships}})
        ...
        current_app.logger.error(f"Error toggling organization relation: {e}", exc_info=True)
        return jsonify({"success": False, "error": f"Failed to toggle organization relation: {str(e)}"}), 500
    ...
    try:
        ...
        if action == 'add':
            if not user_concept_id:
                return jsonify({"success": False, "error": "user_concept_id required for add", "action": action}), 400
        ...
        return jsonify({"success": False, "error": f"Failed to toggle user relation: {str(e)}"}), 500
...
def toggle_key_concept():
    ...
    try:
        ...
        if not target_concept:
            return jsonify({"success": False, "error": f"Concept '{concept_id}' not found"}), 404

        # Verify user concept exists
        user_concept = ConceptsRepository.find_one({"concept_id": user_concept_id})
        if not user_concept:
            return jsonify({"success": False, "error": f"User concept '{user_concept_id}' not found"}), 404

        if action == 'add':
            # Add #V#vontologykeyconcept to is_an_instance_of relationship
            result = ConceptsRepository.update_one(
                {"concept_id": concept_id},
                {"$addToSet": {"relationships.is_an_instance_of": "#V#vontologykeyconcept"}}
            )
            ...
            result = ConceptsRepository.update_one(
                {"concept_id": concept_id},
                {"$pull": {"relationships.is_an_instance_of": "#V#vontologykeyconcept"}}
            )
        ...
        return jsonify({"success": False, "error": f"Failed to toggle key concept: {str(e)}"}), 500
...
def get_key_concepts():
    ...
    try:
        ...
        # Extract concept IDs
        concept_ids = [doc.get("concept_id") for doc in key_concepts if doc.get("concept_id")]
        ...
        return jsonify({"success": False, "error": f"Failed to get key concepts: {str(e)}"}), 500


# ---------------- Propagation (subtree recompute) -----------------
def _recompute_inherited_for_subtree(repo, root_type_id: str, limit: int | None = None, dry_run: bool = False):
    ...
    while queue:
        ...
        if limit and len(order) >= limit:
            break
        try:
            children_cursor = repo.find({"relationships.is_a_type_of": current}, {"concept_id": 1})
            for c in children_cursor:
                cid = c.get("concept_id")
                if isinstance(cid, str) and cid not in visited:
                    queue.append(cid)
        except Exception:
            current_app.logger.debug("Failed fetching children for subtree propagation node %s", current, exc_info=True)

    docs = list(repo.find({"concept_id": {"$in": order}}, {
        "concept_id": 1,
        "relationships": 1,
    ...
    for cid in order:
        ...
        for p in parent_list:
            ...
            if not pd:
                try:
                    pd_full = repo.find_one({"concept_id": p}, {"inherited_salient_binary_predicates": 1, "salient_binary_predicates_for_type": 1})
                except Exception:
                    pd_full = None
                if not pd_full:
                    missing_parent = True
                    continue
                parent_inherited_lists.append(pd_full.get("inherited_salient_binary_predicates") or get_direct(pd_full))
            else:
                parent_inherited_lists.append(pd.get("inherited_salient_binary_predicates") or [])
        if missing_parent:
        ...
        if recomputed != existing:
            ...
            if not dry_run:
                try:
                    repo.update_one({"concept_id": cid}, {"$set": {"inherited_salient_binary_predicates": recomputed}})
                except Exception:
                    current_app.logger.exception("Failed updating inherited salient for %s", cid)
...
def propagate_salient_subtree():
    ...
    try:
        summary = _recompute_inherited_for_subtree(repo, root_type_id, limit=limit, dry_run=dry_run)
    except Exception as e:
