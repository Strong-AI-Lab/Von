...
def has_text_payload(doc: Dict[str, Any]) -> bool:
    ...
    for entry in interactions:
        ...
        if details.get('question') and (details.get('answer') or details.get('answer_preview')):
            return True
...
def main():
    ...
    print(f'  total={total} missing={missing} skipped={skipped} pending={pending} indexed={indexed} failed={failed}')
    ...
    if not to_mark:
        ...
        result = coll.update_many({'_id': {'$in': to_mark}}, {
            '$set': {
                'indexing_status': IndexingStatus.PENDING.value,
                'pending_marked_at': datetime.now(timezone.utc)
            },
            '$unset': {
                'embedding': ''  # ensure re-index if partial data existed
            }
        })
        print(f'Marked {result.modified_count} documents as pending (from candidates={len(to_mark)}).')

    # Post stats
    pending_after = coll.count_documents({'indexing_status': IndexingStatus.PENDING.value})
    skipped_after = coll.count_documents({'indexing_status': IndexingStatus.SKIPPED.value})
