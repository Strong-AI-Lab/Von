...
def upsert_suggested_relations_for_type(subject_type_id: str, predicate_ids: List[str], updated_by: str) -> Dict[str, Any]:
...
def get_suggested_relations_for_type(subject_type_id: str) -> Optional[List[str]]:
    doc = MetaRelationsRepository.find_one({"type": "suggested_relations_for_type", "subject_type_id": subject_type_id})
    if not doc:
        return None
    return doc.get("predicate_ids") or []


def upsert_question_prompt(predicate_id: str, template: str, locale: str, updated_by: str) -> Dict[str, Any]:
    if not predicate_id:
    ...
    MetaRelationsRepository.update_one(
        {"type": "user_question_prompt_for_relation", "predicate_id": predicate_id, "locale": doc["locale"]},
        {"$set": doc},
        upsert=True,
    )
    return doc
...
def upsert_parse_prompt(predicate_id: str, parse_prompt: str, updated_by: str) -> Dict[str, Any]:
    if not predicate_id:
        raise MetaRelationsServiceError("predicate_id required")
    if not parse_prompt:
