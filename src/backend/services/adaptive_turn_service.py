...
def _scope_message(
    ...
    *,
    ...
    message = (
        ...
        "- Treat one schema-discovery read followed by one content-bearing read "
        "as a small composition, not as a reason by itself to select a broader "
        "workflow.\n"
        "- Match evidence coverage to the requested claim. A positive hit proves "
        "existence, not completeness. For a requested list, count, broad category, "
        "or negative claim, inspect enough relevant pages, directions, and "
        "represented schema variants to support that scope, or state the bound; "
        "stop once the evidence supports the requested work product.\n"
        "- Reconcile quantitative summaries with the grounded items. Count only "
        "items evidenced to satisfy the requested category; report related "
        "artefacts or ambiguous candidates separately rather than including them "
        "in the total.\n"
