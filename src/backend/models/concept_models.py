...
class InteractionEntry(BaseModel):
    ...
    details: Dict[str, Any] = {} # E.g., {"question": "What is X?", "answer_preview": "Y..."}
...
class ConceptInteraction(BaseModel):
    ...
    concept_identifier: str # _id from 'concepts' for individuals, Vontology path for types
    concept_vontology_path: List[str]
    concept_kind: ConceptKind
    concept_name_for_display: str # Denormalized for easier querying of interaction history
...
interaction_session_collection_name = "interactions" # Updated to match current db structure
