    ...
    def get_instance(token_path):
        if DriveService._instance is None:
            ...
            try:  # Lazy import to avoid mandatory dependency at import time
                ...
                raise RuntimeError("google-api-python-client not installed. Install to enable Google Drive features.")
...
def authenticate_google_drive(token_path):
    ...
    try:
        ...
        raise RuntimeError("Google Drive dependencies missing. Install 'google-api-python-client google-auth google-auth-oauthlib'.") from e
    ...
    if not creds or not getattr(creds, 'valid', False):
        if creds and getattr(creds, 'expired', False) and getattr(creds, 'refresh_token', None):
            ...
            flow = InstalledAppFlow.from_client_secrets_file(get_drive_cred_name(), SCOPES)
...
def create_token_file(token_path):
    ...
    try:
        ...
        raise RuntimeError("google-auth-oauthlib not installed. Install to create token file.") from e
...
def save_to_drive(location, token_path, file_path, file_name, file_content, mime_type='text/plain'):
    """
...
def iterate_files_in_folder(token_path, folder_id=None):
    ...
    try:
        ...
        results = service.files().list(pageSize=1000, q=query, fields="files(name,id,mimeType)").execute()
