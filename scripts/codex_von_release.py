...
def verify(target, commit):
    ...
    # Older coherent releases predate the shared retry module. They remain valid
    # rollback targets; newer workers must carry their actual imported dependency.
    if (
        "codex_von_retry" in (target / "scripts/codex_von_worker.py").read_text()
        and not (target / "scripts/codex_von_retry.py").is_file()
    ):
        raise runtime.DeploymentError(
