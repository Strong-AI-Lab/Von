# Historical Agent Guidance Notes

**Status**: Historical archive, not authoritative  
**Source**: Former `Backup_AGENTS.md`, preserved on 2026-04-04

This document preserves older agent guidance as historical context and as a
record of practical lessons that previously accumulated in `AGENTS.md`.

It may contain superseded, duplicated, over-detailed, or stale guidance.
Prefer the current authoritative documents:

- `AGENTS.md`
- `docs/engineering/operational_engineering_guide.md`
- the current engineering documents linked from those files

Consult this note only when investigating prior operational failure modes,
...
18C. **Mandatory modern-agentic design review for substantial implementation planning**: before finalising the plan for any substantial implementation task, review `docs/engineering/intro_to_modern_agentic_ai_for_coding_agents.md`. This is especially mandatory for ranking, recommendation, classification, matching, routing, planning, prompting, retrieval, memory/KB, ontology/representation, and evaluation work.
19. **Workflow-first / KB-authoritative**: express behaviour in VWL/Vontology when possible; do not encode workflow definitions, routing, or task-specific orchestration in Python when reusable workflow/runtime surfaces can carry the behaviour. The authoritative authored representation should live in Vontology/KB artefacts, not repo-side workflow files, whenever the behaviour can be represented there. If VWL is insufficient, add only the missing reusable execution/validation/telemetry/persistence surface and document the capability gap in Jira/Vontology.
19A. **Beware repo-declarative masquerading as KB-first**: common training/codebase priors strongly favour "files are the authored source; databases/KBs are runtime state". Replacing Python with repo-side JSON/YAML/DSL workflow files is **not** workflow-first convergence unless those files are only migration seeds, generated snapshots, test fixtures, or exports. Treat any production path that makes such files authoritative as source-authority drift.
19B. **No self-serving workflow-policy relaxation**: the same implementation effort may tighten or clarify workflow-authority guidance, but must not broaden what counts as compliant or weaken KB-authority requirements to fit its own design. Any genuine exception or relaxation requires a separate Jira issue, explicit human approval, and a concrete statement of the missing VWL/Vontology primitive.
19C. **Decision-policy authority extends beyond routing**: ranking, recommendation, matching, selection, classification, explanation, and planning policy count as authored behaviour, not just implementation detail. For open-ended natural-language content, do not ship production policy as lexical heuristics in Python when embedding-based, LLM-based, or workflow/KB-authoritative approaches are available. Treat hard-coded tokenisers, stopword lists, lexical-overlap heuristics, fixed score weights, and threshold tables as a design tripwire, not a default implementation pattern.
19D. **Heuristic fallbacks must be explicitly temporary and non-authoritative**: if a heuristic fallback is temporarily necessary, mark it as temporary and non-authoritative in code and Jira, link a removal or replacement task, state the missing reusable primitive or authority surface, and do not allow the heuristic path to become the primary production decision policy by accident.
...
- `docs/engineering/intro_to_modern_agentic_ai_for_coding_agents.md`: required background review before substantial implementation planning, especially for behaviour/policy/representation-sensitive work.
...
- **Trace sanitisation must preserve benign typed telemetry**: do not blanket-redact or string-coerce non-secret counters such as `prompt_tokens`, `completion_tokens`, `total_tokens`, durations, or similar numeric telemetry fields. Redact secret-bearing values, but keep safe metrics machine-readable so downstream analytics, dashboards, and prediction surfaces remain trustworthy.
...
- **Prompt/workflow-first diagnosis**: when behaviour looks wrong on an LLM- or workflow-governed path, inspect the authoritative Vontology prompts, workflow definitions, routing metadata, and rendered selector/dispatcher inputs before doing code-first diagnosis. Use those artefacts to drive the subsequent code analysis; do not begin from code-only assumptions when prompt/workflow authority may already explain the outcome.
...
- **Workflow-first behaviour checklist (mandatory for behaviour/orchestration changes):**
	0. Before coding, name the intended authoritative KB artefacts in Jira or task notes (workflow concepts, template/profile concepts, prompt concepts, routing metadata, text relations, or other Vontology-native structures) and distinguish them from any supporting code-only surfaces.
	0A. Before coding, decide whether the task is also authoring decision policy (for example ranking, recommendation, matching, classification, planning, explanation, or retrieval strategy). If so, treat that policy as authoritative behaviour that should live in Vontology/workflow/prompt/KB artefacts where feasible, and keep Python focused on reusable support surfaces.
	...
	2B. If a workflow could be implemented by a genuine general extension of VWL rather than a context-specific patch, make that VWL/runtime extension first, then implement the workflow fully in VWL. Do not leave the workflow logic hidden in Python once the behaviour is generalisable as language/runtime capability.
	...
	8. Repo-side workflow/template/prompt files are non-authoritative by default. They are acceptable only as migration seeds, generated snapshots, test fixtures, or exports. If deleting them would remove the authoritative behaviour or logic, the design is not yet KB-authoritative.
- **Workflow-authority readiness/closure checklist (mandatory for workflow-related tasks):**
	- Before implementation, record the authoritative KB artefacts that will hold the logic.
	- Before closure, distinguish the supporting code surfaces from the authoritative KB artefacts.
	- Before closure, verify that any authoritative decision policy lives in Vontology/workflow/prompt/KB artefacts or materialised KB assertions, not in Python helper code or lexical heuristic scaffolding.
	- Before closure, ask whether any docs/manuals became more permissive; if yes, stop and spin that into a separate approved Jira change or exception rather than normalising it inside the implementation task.
	- Before closure, ask whether repo-side snapshots or seed files could be deleted without losing authority; if not, the task is not yet KB-authoritative.
	- If repo-side files remain, state why they are non-authoritative (migration seed, generated snapshot, test fixture, or export) and link the task that removes or supersedes them if they are temporary.
...
- **Task lifecycle discipline**:
	...
	- Treat `git push` as incomplete until you verify the remote ref. Before saying a task is merged or transitioning it to `Done`, confirm `origin/main` contains the task commit(s) or the merge commit that contains them. Do not rely on push output alone; concurrent upstream movement can leave local `main` and `origin/main` diverged.
...
- When asked to "merge", default to: ensure changes are committed, fast-forward merge into `main`, push `main`, verify `origin/main` now contains the intended commit(s), then delete the local and remote feature branch; if deletion needs a force flag, ask for confirmation first.
...
- **If Jira MCP transport is unavailable in-session**: when the `jira_*` tool surface itself is unreachable (for example MCP transport closed) but the local repo/runtime is available, use Von's canonical repo-local Jira proxy implementation rather than inventing an ad-hoc workaround: load `.env` for the process, then call the supported helpers in `src/backend/mcp_server/mcp_server.py` / `src/backend/integrations/internal_mcp/catalogue.py` so auth, ADF conversion, and Jira request shaping stay on the standard path. Do not fall back to handwritten REST scripts.
